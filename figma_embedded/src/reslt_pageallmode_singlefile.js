  ...
  const getThemeColors = useMemo(() => {
    if (theme === 'dark') {
      return {
        ...
        // Updated MAIN_BG to match ChatPage gradient for consistency
        MAIN_BG: 'linear-gradient(to bottom right, #072056, #000B23)', 
...
const MainContent = () => {
  ...
  return (
    <div className="flex flex-col flex-grow h-full overflow-hidden" style={{ background: 'linear-gradient(to bottom right, #072056, #000B23)' }}>
          ...
          >
            ...
            {DATA_TABLE.map((row, index) => (
              <div
                ...
                onMouseEnter={(e) => {
                  // Remove hover background effect for transparent table
                }}
                onMouseLeave={(e) => {
                  // Remove hover background effect for transparent table
                }}
              >
                <span className="w-1/4" style={{ color: '#ffffff' }}>{row.name}</span>
                <span className="w-1/4" style={{ color: '#ffffff' }}>{row.id}</span>
                <span className="w-1/3" style={{ color: '#ffffff' }}>{row.email}</span>
      ...
      <div className="flex-shrink-0 p-4" style={{ borderTop: `1px solid ${colors.BORDER}`, background: 'linear-gradient(to bottom right, #072056, #000B23)' }}>
        <div
          className="mx-auto max-w-5xl flex items-center p-3 rounded-full border"
          style={{ borderColor: colors.PRIMARY_COLOR, background: 'linear-gradient(to bottom right, #072056, #000B23)' }}
        >
          <input
            type="text"
