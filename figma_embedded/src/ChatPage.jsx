...
const ChatPage = ({ onBack, userQuestion, onToggleTheme, isDarkMode, currentThread, isNewChat, isNewChatActive, onNewChat, onThreadSelect, onFirstMessage }) => {
  // Debug: Log the isDarkMode value
  console.log('ChatPage: isDarkMode =', isDarkMode);
  <MenuSidebar 
        onBack={onBack}
        ...
        currentActiveThread={currentThread?.id}
        isNewChatActive={isNewChatActive}
      />;
  console.log('ChatPage: typeof isDarkMode =', typeof isDarkMode);
  
  const [userInput, setUserInput] = useState('');
  const [messages, setMessages] = useState(() => {
    if (userQuestion && !currentThread?.conversation?.length) {
      // Coming from figma-hero page with a question - show table response
      return [
        {
          id: 1,
          type: 'user',
        ...
        },
        {
          id: 2,
          type: 'assistant',
          text: 'Based on the latest records, here is the information you requested:',
          showTable: true
        }
      ];
    } else if (isNewChat) {
    ...
    } else if (currentThread?.conversation) {
  ...
  // Save thread when coming from figma-hero page
  useEffect(() => {
    if (userQuestion && !isNewChat && !currentThread && messages.length >= 2) {
      ...
      // Immediately call onFirstMessage to set this as current thread and enable new chat
      if (onFirstMessage) {
        onFirstMessage(threadToSave);
      }
    }
  }, [userQuestion, isNewChat, currentThread, messages, onFirstMessage]);
  ...
  const generateResponse = (userMessage) => {
    // Simple response logic based on keywords - always return text responses for user inputs
    const lowerMessage = userMessage.toLowerCase();
    
    if (lowerMessage.includes('ticket') || lowerMessage.includes('it service')) {
      return {
        text: 'I\'ll help you create an IT service ticket. Please provide me with the following details: 1) Issue description, 2) Priority level, 3) Department, and I\'ll process your request immediately. You can expect a response within 2-4 hours during business hours.',
        showTable: false
      };
    } else if (lowerMessage.includes('confluence') || lowerMessage.includes('document')) {
      return {
        text: 'I\'ve searched our confluence database and found several relevant documents. Here are the most recent and relevant pages that match your query. The documents include setup guides, troubleshooting steps, and best practices. Would you like me to summarize any specific document or help you find additional resources?',
        showTable: false
      };
    } else if (lowerMessage.includes('team') || lowerMessage.includes('training') || 
               lowerMessage.includes('completion') || lowerMessage.includes('status') || 
               lowerMessage.includes('cyber') || lowerMessage.includes('security')) {
      return {
        text: 'Based on the latest records from our HR system, I can provide you with detailed training completion status for your team members. Currently, there are 5 employees who haven\'t completed the required training, and 19 who have successfully finished. I can help you send reminders or schedule additional training sessions if needed.',
        showTable: false
      };
    } else if (lowerMessage.includes('project') || lowerMessage.includes('update')) {
      return {
        text: 'Here are the latest project updates from our project management system. The Q4 initiatives are progressing well with 75% completion rate. Key milestones achieved this week include infrastructure setup and initial testing phase. Next steps involve user acceptance testing and deployment preparation.',
        showTable: false
      };
    } else if (lowerMessage.includes('jira') || lowerMessage.includes('bug') || lowerMessage.includes('issue')) {
      return {
        text: 'I can help you with Jira-related tasks. Whether you need to create a new ticket, check the status of existing issues, or get reports on project progress, I\'m here to assist. Currently, there are 12 open tickets in your assigned projects, with 3 marked as high priority.',
        showTable: false
      };
    } else if (lowerMessage.includes('servicenow') || lowerMessage.includes('service now')) {
      return {
        text: 'ServiceNow integration is available for various IT service requests. I can help you create incidents, service requests, or check the status of existing tickets. Our average resolution time is 24 hours for standard requests and 4 hours for urgent issues.',
        showTable: false
      };
    } else {
      return {
  ...
  const saveThreadToStorage = (thread) => {
    try {
      ...
      console.log('Thread saved to localStorage:', thread.title);
      console.log('Current threads in today:', threads.today.length);
      
      // Trigger a storage event to update the sidebar immediately
  ...
  // Clear all conversations on component mount/refresh (only once per session)
  useEffect(() => {
    const sessionKey = 'app_session_' + new Date().toDateString();
    const hasBeenCleared = sessionStorage.getItem(sessionKey);
    
    if (!hasBeenCleared) {
      try {
        localStorage.removeItem('chatThreads');
        sessionStorage.setItem(sessionKey, 'true');
        console.log('Cleared all conversations on app start');
      } catch (error) {
  ...
  const handleSendMessage = () => {
    if (userInput.trim()) {
      ...
      // All user inputs get text responses (no table for follow-up questions)
      ...
      const assistantResponse = {
        ...
        type: 'assistant',
        text: response.text,
        showTable: response.showTable
      };
      
      const newMessages = [...messages, userMessage, assistantResponse];
      setMessages(newMessages);
      
      // If this is a new chat, create a new thread in "today" category
      if (isNewChat && messages.length <= 1) {
        ...
        const updatedThread = {
          ...currentThread,
          title: currentThread.title === 'New Chat' && userInput ? userInput : currentThread.title,
          conversation: newMessages.map(msg => ({
            type: msg.type,
            text: msg.text,
            showTable: msg.showTable || false,
        ...
        // Create thread for figma-hero question in "today" category
        const newThread = {
          id: 'thread_' + Date.now(),
          title: userQuestion.length > 50 ? userQuestion.substring(0, 50) + '...' : userQuestion,
          conversation: newMessages.map(msg => ({
  ...
  // Updated the onNewChat handler to open a new chat with help text and set the title as 'New Chat'
  const handleNewChat = () => {
    const newChatMessage = {
      id: 1,
      type: 'assistant',
      text: 'Welcome to the new chat! How can I assist you today?',
      isWelcome: true
    };
    ...
    // Reset isNewChatActive to allow further new chats
    if (isNewChatActive) {
      onNewChat(false);
    }
  };
      ...
      <div className={`flex-1 flex flex-col h-screen relative z-10 ${isDarkMode ? 'bg-gradient-to-br from-[#072056] to-[#000B23]' : ''}`}>
              ...
              <div key={message.id} style={{ width: '100%' }}>
                {message.type === 'user' ? (
                  ...
                  <div className="space-y-4">
                    <div className={`${isDarkMode ? 'text-[#A0BEEA]' : 'text-[#787777]'} text-sm leading-6`}>
                      {message.text}
                    </div>
                    ...
                    {message.suggestions && (
                      <div className="grid grid-cols-1 gap-2">
                        {message.suggestions.map((suggestion, index) => (
                          <div key={index} className={`${isDarkMode ? 'bg-[#1F3E81] text-[#A0BEEA]' : 'bg-[#F7F7F7] text-[#333333]'} rounded p-3 text-sm`}>
                            • {suggestion}
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {message.showTable && (
                      <div className={`${isDarkMode ? 'bg-transparent border-transparent' : 'bg-white border-gray-200'} rounded-lg border overflow-hidden shadow-sm`}>
                        <table className="w-full text-sm">
                          ...
                          <tbody className={`${isDarkMode ? 'divide-transparent' : 'divide-gray-200'} divide-y`}>
                            <tr>
                              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Deepl, Priya</td>
                              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>AGT23456</td>
                              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>Priya.Deepl@elevancehealth.com</td>
                            ...
                            <tr>
                              ...
                              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>AGT23456</td>
                              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>Sophia.Garcia@elevancehealth.com</td>
                              <td className="px-4 py-3">
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${isDarkMode ? 'bg-red-900 text-red-200' : 'bg-red-100 text-red-800'}`}>
                                  Not Started
                                </span>
                            ...
                            <tr>
                              ...
                              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>AGT23456</td>
                              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>Alex.Johnson@elevancehealth.com</td>
                              <td className="px-4 py-3">
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${isDarkMode ? 'bg-red-900 text-red-200' : 'bg-red-100 text-red-800'}`}>
                                  Not Started
                                </span>
                            ...
                            <tr>
                              ...
                              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>AGT23456</td>
                              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>Marco.Lin@elevancehealth.com</td>
                              <td className="px-4 py-3">
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${isDarkMode ? 'bg-red-900 text-red-200' : 'bg-red-100 text-red-800'}`}>
                                  Not Started
                                </span>
                            ...
                            <tr>
                              ...
                              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>AGT23456</td>
                              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>Ethan.Miller@elevancehealth.com</td>
                              <td className="px-4 py-3">
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${isDarkMode ? 'bg-red-900 text-red-200' : 'bg-red-100 text-red-800'}`}>
                                  Not Started
                                </span>
        ...
        <div className={`p-5 ${isDarkMode ? 'bg-gradient-to-br from-[#072056] to-[#000B23]' : 'bg-[#F9FAFB] border-gray-200 border-t'} flex-shrink-0`}>
          <div className="max-w-[760px] mx-auto">
            <div 
              className={`flex items-center gap-3 px-4 py-3 border-2 ${isDarkMode ? 'border-[#2861BB] rounded-full' : 'bg-white border-[#44B8F3] rounded-full'}`}
              style={isDarkMode ? {
                borderRadius: '999px',
