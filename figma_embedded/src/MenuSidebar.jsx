...
import { Search, Plus, ArrowLeft, Sun } from 'lucide-react';
    ...
    <div className={`${isCollapsed ? 'w-[80px]' : 'w-[295px]'} ${isDarkMode ? 'bg-[#03112F]' : 'bg-white'} flex flex-col h-screen shadow-[0_20px_40px_0_rgba(0,47,189,0.10)] transition-all duration-300`}>
      ...
      <div className="flex flex-col px-4 pt-6 pb-4 gap-6 flex-shrink-0">
        ...
        <div className="flex h-10 items-center justify-between">
          ...
          {/* Menu Icon - Clickable */}
          <button onClick={() => setIsCollapsed(!isCollapsed)} className="flex-shrink-0 p-2 hover:opacity-70 flex items-center justify-center">
        ...
        <div className="flex items-center justify-between">
          {/* Back Button - Always show icon, hide text when collapsed */}
          <button 
            className={`flex items-center hover:opacity-70 ${isCollapsed ? 'justify-center p-2' : 'gap-3 flex-1'}`}
            onClick={onBack}
          >
          ...
          {!isCollapsed && (
            <button
              ...
              style={{ borderColor: '#1A3673', backgroundColor: '#FFF' }}
            >
              {/* Toggle Circle */}
              <div
        ...
        {isCollapsed ? (
          <div className="flex justify-center">
            <button className="p-2 hover:opacity-70 flex items-center justify-center">
              <Search className="w-6 h-6" style={{ color: isDarkMode ? '#FFF' : '#949494' }} />
            </button>
          </div>
        ) : (
          <div className="flex flex-col justify-center items-start h-10 px-3 py-2 rounded border" style={{ borderColor: isDarkMode ? '#444' : '#CCC' }}>
            <div className="flex items-center gap-2 w-full">
              <Search className="w-5 h-5" style={{ color: isDarkMode ? '#FFF' : '#949494' }} />
              <span style={{ color: isDarkMode ? '#FFF' : '#2861BB', fontSize: '16px', fontWeight: 500, lineHeight: '16px' }}>
                Search
              </span>
            </div>
          </div>
        ...
        <div className="flex flex-col items-start gap-4 mb-6">
          ...
          <div className="flex flex-col items-start gap-6 w-full">
            ...
            {allThreads.today.length > 0 && (
              <div className="flex flex-col items-start gap-1 w-full">
                ...
                <div className="flex flex-col items-start gap-2 w-full">
                  {allThreads.today.map((thread, index) => (
                    <button 
                      key={index} 
            ...
            <div className="flex flex-col items-start gap-1 w-full">
              ...
              {/* Thread Items */}
              <div className="flex flex-col items-start gap-2 w-full">
                {allThreads.lastWeek.map((thread, index) => (
                  <button 
                    key={index} 
                    className={`flex items-center gap-2 w-full p-2 rounded transition-colors ${
                      currentActiveThread?.id === thread.id 
            ...
            <div className="flex flex-col items-start gap-1 w-full">
              ...
              {/* Thread Items */}
              <div className="flex flex-col items-start gap-2 w-full">
                {allThreads.last30Days.map((thread, index) => (
                  <button 
                    key={index} 
                    className={`flex items-center gap-2 w-full p-2 rounded transition-colors ${
                      currentActiveThread?.id === thread.id 
      ...
      >
        <button
          ...
          className={`flex items-center justify-center gap-1 transition-colors ${
            isCollapsed ? 'w-10 h-10' : (isDarkMode ? '' : 'w-full h-10')
          } ${
            isNewChatActive 
              ? 'bg-gray-300 cursor-not-allowed' 
              : (isDarkMode ? '' : 'bg-[#2861BB] hover:bg-[#1f4a9c]')
          }`}
          style={isDarkMode && !isCollapsed ? {
            display: 'flex',
            width: '269px',
            height: '40px',
            padding: '4px 16px',
            ...
            height: '40px',
            borderRadius: '4px',
            background: isDarkMode ? '#FFF' : (isNewChatActive ? '#gray-300' : '#2861BB')
          } : {}}
        >
          <Plus className="w-4 h-4" style={{ color: isDarkMode ? '#000' : (isNewChatActive ? '#999' : '#FFF') }} />
          {!isCollapsed && (
            <span style={{ color: isDarkMode ? '#000' : (isNewChatActive ? '#999' : '#FFF'), fontSize: '14px', fontWeight: 600 }}>
              {isNewChatActive ? 'New chat active...' : 'New chat'}
            </span>
          )}
        </button>
      </div>
