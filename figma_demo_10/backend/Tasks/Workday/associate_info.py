...
from modules.workflow_utils import replace_short_names,extract_date_parts,get_workday_message_et
import json
...
def get_employeeinfo(state: AgentState):
    ...
    logger.info(f'associate prompt:{prompt}')
...
def get_employeeinfo_prompt(state):
    ...
    if response_emp is None:
        store=store_associateinfo(domainid)
        response_emp = redis_client.hget(f"in_demographicinfo:{domainid}", "data")
    ...
    as_of_date=get_workday_message_et()

    date_context = get_date_context()

    flags = {
        "include_birthdays": state.get("include_birthdays") == "yes",
        "include_anniversaries": state.get("include_anniversaries") == "yes",
        "include_job_profile_duration": state.get("include_job_profile_duration") == "yes"
    }
    prompt = generate_combined_prompt(flags, response_emp, as_of_date, date_context)
    return prompt

def transform_employee_data(response_emp, task_types=None):
    def extract_info(person, include_all=False):
        if include_all:
            return person  # Return full data if no task match
        ...
        info = {
            ...
            "Domain_ID": person.get("Domain_ID", ""),
            "Business_Title": person.get("Business_Title", ""),
            "Employee_Type": person.get("Employee_Type", ""),
            "Hire_Date": person.get("Hire_Date", ""),
            "cost center": person.get("cost center", ""),
            "supervisory organization": person.get("supervisory organization", ""),
            "Job_Family": person.get("Job_Family", ""),
            "Work_Location_Description": person.get("Work_Location_Description", "")
        }

        # Add task-specific fields
        if "birthdays" in task_types:
            info["Date_of_Birth"] = person.get("Date_of_Birth", {})
        if "anniversaries" in task_types:
            info["Continuous_Service_Date"] = person.get("Continuous_Service_Date", {})
        if "job_profile_duration" in task_types:
            info["Time_in_Job_Profile"] = person.get("Time_in_Job_Profile", {})

        return info

    # Determine if task_types are valid
    valid_tasks = {"birthdays", "anniversaries", "job_profile_duration"}
    task_types = task_types or []
    matched_tasks = [task for task in task_types if task in valid_tasks]
    include_all = not matched_tasks

    raw_subordinates = response_emp.get("subordinates", "[]")

    if isinstance(raw_subordinates, str):
        try:
            raw_subordinates = json.loads(raw_subordinates)
        except json.JSONDecodeError:
            raw_subordinates = []

    return {
        "myself": extract_info(response_emp.get("associate", {}), include_all),
        "subordinates": [extract_info(sub, include_all) for sub in raw_subordinates]
    }

def get_template_notes(topics):
    notes_map = {
        "job_profile_duration": '''- For job profile queries, include both the **duration** and the **current position title** (e.g., Business Title or Job Title).
...
- If the user asks about their own role, respond with: "You have been in your current position (**<Job_Title>**) for <duration>."''',
        "birthdays": '''- For birthday queries, check if "Date_of_Birth" matches today's date.
- If it does, respond with: "**<First_Name>**'s birthday is today." or "Happy Birthday, **<First_Name>**!"''',
        "anniversaries": '''- For work anniversary queries, check if "Continuous_Service_Date" matches today's date.
- If it does, respond with: "**<First_Name>**'s work anniversary is today." or "Happy Work Anniversary, **<First_Name>**!"'''
    }

    return "\n\n".join([notes_map.get(topic, "") for topic in topics if notes_map.get(topic)])


def generate_date_context_block(date_context, selected_topics):
...
- “Next 3 months” refers to the next three calendar months.'''
    ...
    return f'''Date context:
            ...
            - The previous month is {date_context["previous_month"]}.
            - The next month is {date_context["next_month"]}.
...
def generate_combined_prompt(flags, response_emp, as_of_date, date_context):
    selected_topics = [key.replace("include_", "") for key, value in flags.items() if value]
    # print('selected_topics:',selected_topics)
    # print('selected_topics type:',type(selected_topics))

    # response_emp=transform_employee_data(response_emp,selected_topics)

    if not selected_topics:
        print('going to default one')
        return generate_prompt("default", response_emp, as_of_date, date_context)

    if len(selected_topics) == 1:
        print('going to selected one:',selected_topics[0])
        return generate_prompt(selected_topics[0], response_emp, as_of_date, date_context)

    topic_labels = {
        "birthdays": "birthday",
        "anniversaries": "work anniversary",
        "job_profile_duration": "job profile duration"
    }
    readable_topics = [topic_labels.get(t, t) for t in selected_topics]

    combined_intro = f"You are a friendly, helpful conversational agent for inquiries about {', '.join(readable_topics)} information for the userand their direct reportees."
    template_notes = get_template_notes(selected_topics)
    date_context_block = generate_date_context_block(date_context, selected_topics)

    return f'''{combined_intro}

Use only the provided JSON object to generate your response. Respond strictly based on the data available. Do not infer or fabricate any information.
...
- Do not mention the JSON, the data source, or any technical reference.
- Answer as a sentence, unless the query requires a different format as specified below.
- If the user's query cannot be fully answered using the available JSON, acknowledge the limitation politely and offer a helpful next step.
- If appropriate, suggest checking with their manager or, only when clearly needed, mention that a live support agent may be able to assist further. Use varied, empathetic phrasing. Do not mention “live agent” unless necessary.
-include the latest data timestamp at the end of the response,using below.
    -Choose the most natural and context-appropriate phrasing for the timestamp, such as:
        - '<i> \\n* As of {as_of_date} from Workday </i>'
        - '<i> \\n* Last updated from Workday at {as_of_date} </i>'
        - '<i> \\n* The above data is current from Workday as of {as_of_date} </i>'
...
- Adapt your response format to the user's query:
  ...
  - Emphasis: Use **bold text** for important keywords or phrases.
...
def generate_prompt(template_type, response_emp, as_of_date, date_context):
    ...
    if template_type == "birthdays":
    ...
    elif template_type == "anniversaries":
        ...
        job_profile_note = template_notes.get(template_type, "")
    elif template_type == "job_profile_duration":
        ...
        return f'''You are a friendly, helpful conversational agent for inquiries about associate information(myself, my direct reportees). Basedon the given JSON object.
        ...
        In the Demographic JSON, 'associate' refers to my details, and 'subordinate' refers to my direct reportees.ManagerID refers to  Manager Doimain ID.You must consider 'all' the json elements 'beginning to end; to provide your response, and list all the matching entries, NOT just few.
