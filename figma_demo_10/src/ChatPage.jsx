...
// API endpoints are imported below in this file
import { cleanStreamText, parseCaseFlagsFromText, buildCaseCreationBlock } from './utils/workforceAgentUtils';
...
import { renderLiveAgentMessage as renderLiveAgentMessageUtil } from './utils/messageFormatter';
...
const formatTextWithLinks = (text) => {
  ...
  // Special handling for persisted system markers: render styled components and never show raw tokens
  try {
    // Match system markers and optionally capture a third value (e.g., selected key)
    const sysMarkerMatch = text.match(/<<\s*\[\s*"?system"?\s*,\s*"?([^"\]]+)"?\s*(?:,\s*"?([^"\]]+)"?)?\s*\]\s*>>/i);
    if (sysMarkerMatch) {
      const marker = (sysMarkerMatch[1] || '').toLowerCase();
      const extra = (sysMarkerMatch[2] || '').toLowerCase();
      // Connecting to live agent banner
      if (marker === 'connecting_to_live_agent') {
        // Use the same visual component as other live agent messages for consistency
        // Detect dark mode from DOM since we're outside the component scope
        let isDark = false;
        try {
        ...
        return renderLiveAgentMessageUtil('Connecting you with HR Support... Please wait while we establish the session.', true, 'HR Support', isDark);
      }
      // End of conversation banner: match terminateLiveAgent style (plain inline text, no bubble)
      if (marker === 'end_conversation' || marker === 'session_ended') {
        // Use the third captured value as the reason, fallback to a generic message
        const reason = (sysMarkerMatch[2] || '').trim() || 'Live-agent session ended.';
        // Match the terminateLiveAgent UX copy
        const finalText = `${reason} You can continue chatting with the WorkPal.`;
        // Return plain text so it renders like terminateLiveAgent system line (see screenshot)
        return finalText;
      }
      // Continue chatting marker (optional subtle banner)
      if (marker === 'continue_chatting' || marker === 'you can continue chatting here with workpal' || marker === 'continue_chatting_workpal') {
        // Remove border, transparent background; dark mode -> white text; light mode -> keep same color
        let isDark = false;
        try {
          isDark = document.body.classList.contains('dark') ||
                   document.documentElement.classList.contains('dark') ||
        ...
        const textColorClass = isDark ? 'text-white' : 'text-[#22543d]';
        return (
          <div className={`system-banner continue-chatting flex items-center gap-2 rounded-[10px] px-3 py-2 text-sm leading-snug bg-transparent ${textColorClass}`}>
            <span><strong className="font-semibold">You can continue chatting here with WorkPal.</strong> Ask a question to continue.</span>
          </div>
        );
      }
      // ServiceNow catalog marker: render instruction and button to open URL
      if (marker === 'servicenow_catalog' || marker === 'servicenow catalog newtab' || marker === 'servicenow_catalog_newtab' || marker === 'servicenow catelog newtab') {
        // Third value may contain a URL
        const catalogUrl = sysMarkerMatch[2] || API_ENDPOINTS.SERVICENOW_HR_CATALOG;
        let isDark = false;
        try {
          isDark = document.body.classList.contains('dark') || document.documentElement.classList.contains('dark') || document.body.getAttribute('data-theme') === 'dark';
        } catch (_) {}
        return (
          <div className={`p-3 rounded-lg ${isDark ? 'bg-transparent' : 'bg-[#f8f9fa]'}`} style={{ textAlign: 'left' }}>
            <div style={{ marginBottom: '12px', color: isDark ? '#9CA3AF' : '#6B7280' }}>
              Use the button below to open the ServiceNow HR catalog in a new tab.
            </div>
            <button
              onClick={() => window.open(catalogUrl, '_blank')}
              className={`button-row-action ${isDark ? 'force-white-text' : 'force-blue-text'}`}
              style={{
                display: 'inline-flex',
                alignItems: 'center',
                gap: '8px',
                padding: '10px 16px',
                borderRadius: '10px',
                border: isDark ? '1px solid #2861BB' : '1px solid #dbe2ea',
                backgroundColor: isDark ? '#122F65' : 'transparent',
                fontWeight: 600,
                cursor: 'pointer'
              }}
            >
              <img src={ServiceNowIconImg} alt="ServiceNow" style={{ height: 18, width: 18, objectFit: 'contain' }} />
              ServiceNow ticket catalog
            </button>
          </div>
        );
      }
      // Button-selected marker: render a static ButtonRow-style replica with selected highlighted and all disabled
      if (marker === 'button_selected') {
        // Normalize various persisted keys to our internal option keys
        const normalizeSelectedKey = (raw) => {
          const k = (raw || '').toLowerCase();
          if (!k) return 'agentichradvisor';
          // Accept multiple alias spellings
          if (k === 'agentichradvisor' || k === 'managercoaching' || k === 'hradvisor') return 'agentichradvisor';
          if (k === 'agenticcontactcenter' || k === 'otherhrsupport' || k === 'contactcenter') return 'agenticcontactcenter';
          if (k === 'servicenowcatalog' || k === 'service_now_catalog' || k === 'servicenow_catalog') return 'servicenow_catalog';
          if (k === 'continuechatting' || k === 'continue_chat' || k === 'continue') return 'continue_chat';
          return k; // fallback to raw, may still match
        };
        // Map known keys to labels; default to first option if unknown
        const selectedKey = normalizeSelectedKey(extra) || 'agentichradvisor';
        // Detect dark mode from DOM (no hooks available here)
        let isDark = false;
        try {
          isDark = document.body.classList.contains('dark') ||
                   document.documentElement.classList.contains('dark') ||
        ...
        const options = [
          { key: 'agentichradvisor', label: 'Manager coaching and coaching for corrective action' },
          { key: 'agenticcontactcenter', label: 'Other HR support' },
          { key: 'servicenow_catalog', label: 'ServiceNow ticket catalog' },
          { key: 'continue_chat', label: 'Continue chatting' }
        ];
  const isSelected = (k) => k === selectedKey;
        return (
          <div className="flex flex-col items-center mt-2">
            {/* Dark mode container wrapper for replayed instruction + buttons */}
            <div>
              <style>{`
              ...
              `}</style>
              <div
                className={`w-full max-w-[680px] rounded-[10px] ${isDark ? 'bg-[#0f2c63] border border-[#2e5dbb] force-white-text' : 'force-blue-text'} p-5 flex flex-col items-start`}
              >
              {/* Instructional text for static replay (does not affect live interactive buttons) */}
              <div className="w-full max-w-[640px] text-[14px] leading-relaxed mb-3 px-1 text-left">
                <p className="mb-3">
                  ...
                  <strong className="font-extrabold">"Manager coaching and coaching for corrective action"</strong>.
                </p>
                <p className="mb-3">
                  If you have questions related to all other HR areas and wish to connect with a live agent, please click{' '}
                  <strong className="font-extrabold">"Other HR support"</strong>.
                </p>
                <p className="mb-3">
                  If you would like to view the HR Service Catalog, please click on{' '}
                  <strong className="font-extrabold">"ServiceNow ticket catalog"</strong>.
                </p>
              </div>
              <div className="flex w-full max-w-[640px] flex-col mt-2">
              {options.map(opt => (
                <button
                  key={opt.key}
                  disabled
                  className={`w-full text-left py-[14px] px-5 mb-3 rounded-[10px] transition-all cursor-not-allowed text-[#1a366f] ${
                    isSelected(opt.key)
                      ? 'border-2 border-[#1a366f] ring-2 ring-[#cddff6] bg-transparent font-semibold'
                      : 'border border-[#dbe2ea] bg-transparent font-medium'
                  }`}
                >
                  {opt.label}
                </button>
      ...
      // Unknown system marker: suppress raw display
  ...
  // Also strip any raw system markers so they never leak into UI
  formattedText = formattedText.replace(/<<\s*\[\s*"system"\s*,\s*"[^"]+"\s*\]\s*>>/gi, '');
...
const ChatPage = ({ onBack, userQuestion, onToggleTheme, isDarkMode, currentThread, isNewChat, isNewChatActive, isNewChatDisabled, onNewChat, onThreadSelect, onFirstMessage, onThreadUpdate, userInfo, addConversationImmediateRef, onAutoSendStart, onAutoSendComplete }) => {
  ...
  let userMessage, botMessage;
    ...
    const getInitialTitle = async (question) => {
      ...
      // Use longer timeout to get high-quality title on first attempt (eliminates need for retry)
      const timeoutMs = 5000; // 5 seconds - sufficient for title generation API
  ...
  if (shouldCreateNewConversation) {
        ...
        console.log('🎯 Getting initial title for:', inputText.substring(0, 50));
        const initialTitle = await getInitialTitle(inputText);
        console.log('✅ Initial title selected:', initialTitle);
  ...
  if (currentThread && currentThread.title === 'New Chat') {
          ...
          console.log('🔧 Creating backend conversation with title:', initialTitle);
          ...
          if (currentThread && backendConversationId) {
            ...
            currentThread.title = initialTitle; // Update title immediately
            console.log('🔄 [DEBUG] Updated thread ID and title', { 
              oldId: oldThreadId, 
            ...
            // Update sidebar to use the new conversation ID and title
            if (addConversationImmediateRef.current) {
              addConversationImmediateRef.current.updateId(oldThreadId, backendConversationId, initialTitle);
            }
            
            // Notify parent of the ID change with updated title
            if (onThreadUpdate) {
              onThreadUpdate({ ...currentThread, id: backendConversationId, title: initialTitle });
            }
          }
          
          // ✅ Title already sent during conversation creation
          // But if API is still generating a better title, update frontend when it arrives
          if (backendConversationId && initialTitle) {
            ...
            // Continue monitoring for better API title (non-blocking)
            generateConversationTitle(inputText, conversationStorage.defaultUserId).then(apiTitle => {
              if (apiTitle && apiTitle !== initialTitle && apiTitle.trim().toLowerCase() !== 'new chat') {
                console.log('🎯 Better API title received:', apiTitle, '(was:', initialTitle + ')');
                // Update frontend only - backend already has initial title
                if (addConversationImmediateRef.current) {
                ...
                console.log('✅ Sidebar title updated to:', apiTitle);
              }
            }).catch(err => console.warn('⚠️ API title generation continued but failed:', err));
          }
  ...
  const [messages, setMessages] = useState(() => {
    ...
    if (currentThread?.conversation && currentThread.conversation.length > 1) {
      return currentThread.conversation.map((msg, index) => ({
        id: index + 1,
        type: msg.type,
        // Use formatter so system markers render as styled banners or get stripped
        text: msg.type === 'system' ? formatTextWithLinks(msg.text) : msg.text,
  ...
  // Guard: ensure any system messages in state never show raw tokens
  useEffect(() => {
    try {
      setMessages(prev => prev.map(m => {
        if (m?.type === 'system') {
          const t = m.text;
          // If text is a string and contains a system marker, pass through formatter (which strips/renders banners)
          if (typeof t === 'string' && /<<\s*\[\s*"system"[\s\S]*?\]\s*>>/i.test(t)) {
            return { ...m, text: formatTextWithLinks(t) };
          }
        }
        return m;
      }));
  ...
  // Stronger sanitizer: on any message list change, remove raw system tokens if they still leak through
  useEffect(() => {
    try {
      setMessages(prev => prev.map(m => {
        if (m?.type === 'system' && typeof m.text === 'string') {
          // Prefer formatting first so we render banners/replicas instead of removing content
          const containsToken = /<<\s*\[\s*"?system"?[\s\S]*?\]\s*>>/i.test(m.text);
          if (containsToken) {
            const formatted = formatTextWithLinks(m.text);
            // If formatter returns a React node, use it; else fall back to stripped text
            if (formatted && typeof formatted !== 'string') {
              return { ...m, text: formatted };
            }
            const stripped = (formatted || m.text).replace(/<<\s*\[\s*"?system"?[\s\S]*?\]\s*>>/gi, '').trim();
  ...
  const buildInitialAgentPayload = ({ requestId, domainid, email, username, agentGroup = 'AgenticHRAdvisor', topicName = 'AgenticHRAdvisor' }) => ({
    requestId,
    token: 'vaacubed',
    botToBot: true,
    clientSessionId: '',
  ...
  // Build follow-up payload for user typed messages
  const buildFollowupAgentPayload = ({ requestId, domainid, email, text }) => ({
    requestId,
    token: 'vaacubed',
    botToBot: true,
    clientSessionId: '',
    silentMessage: false,
    message: { text, typed: true },
    userId: domainid,
    emailId: email,
    timestamp: Date.now(),
    timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'America/New_York'
  ...
  // POST to USER_TO_AGENT API
  const postUserToAgent = async (payload) => {
    // Instrumentation: log outbound payload to live-agent API (sanitized preview)
    try {
      const preview = (() => {
        const clone = { ...payload };
        if (clone.message && typeof clone.message.text === 'string') {
          clone.message = { text: clone.message.text.substring(0, 200), typed: !!clone.message.typed };
        }
        return clone;
      })();
      console.log('[LiveAgent] ➡️ USER_TO_AGENT payload:', preview);
    } catch (_) {}
    const url = API_ENDPOINTS.USER_TO_AGENT;
    ...
    if (!res.ok) {
      ...
      throw new Error(`User->Agent POST failed: ${res.status} ${t}`);
    }
    return res.json().catch(() => ({}));
  };

  // Start live agent for Manager coaching option
  const startManagerCoachingAgent = async () => {
    try {
      const domainid = RESOLVED_DOMAIN_ID;
      if (!domainid) return;
      const info = JSON.parse(localStorage.getItem('userInfo') || '{}');
      const email = info.email || info.emailId || info.mail || '';
      const username = info.preferred_username || info.name || info.username || '';
      const requestId = `REQ-${crypto.randomUUID?.() || (Date.now().toString(36) + Math.random().toString(36).slice(2))}`;
      requestIdRef.current = requestId;

      // Persist system markers for selection and connecting
      try {
        const conversationId = hybridChatService.getCurrentConversationId();
        if (conversationId) {
          await conversationStorage.addMessage(
            conversationId,
            'system',
            `<<["system","button_selected","AgenticHRAdvisor"]>>`,
            { domain_id: domainid, request_id: requestId },
            [],
            `system_${Date.now()}_${Math.random().toString(36).slice(2)}`
          );
          await conversationStorage.addMessage(
            conversationId,
            'system',
            `<<["system","connecting_to_live_agent"]>>`,
            { domain_id: domainid, request_id: requestId },
            [],
            `system_${Date.now()}_${Math.random().toString(36).slice(2)}`
          );
        ...
        console.warn('⚠️ Failed to persist system markers (HRAdvisor):', persistErr);
      }

      // Initial handshake
  const initialPayload = buildInitialAgentPayload({ requestId, domainid, email, username, agentGroup: 'AgenticHRAdvisor', topicName: 'AgenticHRAdvisor' });
      try {
        console.log('[LiveAgent] ⚙️ Starting AgenticHRAdvisor with payload:', { ...initialPayload, message: { ...initialPayload.message, text: String(initialPayload.message?.text || '').substring(0, 100) } });
      } catch (_) {}
      await postUserToAgent(initialPayload);

      // Flag live agent and open websocket
      setLiveAgent(true);
      setIsLiveAgentCardShowing(false);
      // Reuse existing websocket routine in this file
      try { connectWebSocket(requestId, domainid); } catch (e) { console.warn('connectWebSocket not ready:', e); }

      // Optional: show a system banner locally
      try {
        setMessages(prev => ([
          ...prev,
          {
            id: prev.length + 1,
            type: 'system',
            text: renderLiveAgentMessageUtil('Connecting you with an HR Advisor...', true, 'Agentic HR Advisor', isDarkMode),
            completed: true
          }
        ]));
      } catch (_) {}
    } catch (e) {
      console.error('Failed to start live agent:', e);
    }
  };

  // Start live agent for Other HR support (AgenticContactCenter)
  const startOtherHRSupportAgent = async () => {
    try {
      const domainid = RESOLVED_DOMAIN_ID;
      if (!domainid) return;
      const info = JSON.parse(localStorage.getItem('userInfo') || '{}');
      ...
      try {
        ...
        if (conversationId) {
          await conversationStorage.addMessage(
            conversationId,
            'system',
            `<<["system","button_selected","AgenticContactCenter"]>>`,
            { domain_id: domainid, request_id: requestId },
            [],
            `system_${Date.now()}_${Math.random().toString(36).slice(2)}`
          );
        ...
        console.warn('⚠️ Failed to persist system markers (ContactCenter):', persistErr);
      }

      const initialPayload = buildInitialAgentPayload({ requestId, domainid, email, username, agentGroup: 'AgenticContactCenter', topicName: 'AgenticContactCenter' });
      try {
        console.log('[LiveAgent] ⚙️ Starting AgenticContactCenter with payload:', { ...initialPayload, message: { ...initialPayload.message, text: String(initialPayload.message?.text || '').substring(0, 100) } });
      } catch (_) {}
      await postUserToAgent(initialPayload);
      ...
      try {
        setMessages(prev => ([
          ...prev,
          {
            id: prev.length + 1,
            type: 'system',
            text: renderLiveAgentMessageUtil('Connecting you with HR Support...', true, 'HR Support', isDarkMode),
            completed: true
          }
        ]));
      } catch (_) {}
    } catch (e) {
      console.error('Failed to start live agent (Other HR support):', e);
    }
  };

  // Send user text to live agent while session is active
  const sendMessageToLiveAgent = async (text) => {
    try {
      const domainid = RESOLVED_DOMAIN_ID;
      const info = JSON.parse(localStorage.getItem('userInfo') || '{}');
      const email = info.email || info.emailId || info.mail || '';
      const requestId = requestIdRef.current || `REQ-${Date.now()}`;
      const payload = buildFollowupAgentPayload({ requestId, domainid, email, text });
      try {
        console.log('[LiveAgent] ✉️ Follow-up USER_TO_AGENT payload:', { ...payload, message: { text: String(text || '').substring(0, 200), typed: true } });
      } catch (_) {}

      // Persist the user message with constrained type while live agent is active
      try {
        const conversationId = hybridChatService.getCurrentConversationId();
        if (conversationId && liveAgent) {
          const chatId = `user_${Date.now()}_${Math.random().toString(36).slice(2)}`;
          await conversationStorage.addMessage(
            conversationId,
            'user',
            text,
            { source: 'chat_page', domain_id: domainid, live_agent_enabled: true, request_id: requestId },
            [],
            chatId
          );
        }
      } catch (persistErr) {
        console.warn('⚠️ Failed to persist live-agent user message:', persistErr);
        try {
          const conversationId = hybridChatService.getCurrentConversationId();
          if (conversationId) {
            await conversationStorage.addMessage(
              conversationId,
              'system',
              `<<["system","technical_error","persist_user_message_failed"]>>`,
              { domain_id: domainid, request_id: requestId },
              [],
              `system_${Date.now()}_${Math.random().toString(36).slice(2)}`
            );
  ...
  // Handle routing requested by ButtonRow
  const onButtonRowRoute = async (groupName) => {
    if (groupName === 'AgenticHRAdvisor') return startManagerCoachingAgent();
    if (groupName === 'AgenticContactCenter') return startOtherHRSupportAgent();
  };
  ...
  // Removed local renderLiveAgentMessage in favor of shared util (imported as renderLiveAgentMessageUtil)

  // Sticky banner for live agent session (AI chat untouched)
  const LiveAgentBanner = () => {
  ...
  const terminateLiveAgent = async (reason = "Disconnected from live agent.", domainidParam) => {
    try {
      ...
      setMessages(prev => ([...prev, {
        id: prev.length + 1,
        type: 'system',
        text: `${reason} You can continue chatting with the WorkPal.`
      }]));
    } catch (e) {
  ...
  const handleLiveAgentConnect = async (info) => {
    if (!info) return;

    if (info.type === 'servicenow_catalog') {
      const label = info.label || 'ServiceNow ticket catalog';
      const url = info.url || API_ENDPOINTS.SERVICENOW_HR_CATALOG;
      const bubble = (
        <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-transparent' : 'bg-[#f8f9fa]'}`} style={{ textAlign: 'left' }}>
          <div style={{ marginBottom: '12px', color: isDarkMode ? '#9CA3AF' : '#6B7280' }}>
            Use the button below to open the ServiceNow HR catalog in a new tab.
          </div>
          <button
            onClick={() => window.open(url, '_blank')}
            className={`button-row-action ${isDarkMode ? 'force-white-text' : 'force-blue-text'}`}
            style={{
              display: 'inline-flex',
              alignItems: 'center',
              gap: '8px',
              padding: '10px 16px',
              borderRadius: '10px',
              border: isDarkMode ? '1px solid #2861BB' : '1px solid #dbe2ea',
              backgroundColor: isDarkMode ? '#122F65' : 'transparent',
              fontWeight: 600,
              cursor: 'pointer'
            }}
          >
      ...
      setMessages(prev => ([
        ...prev,
        { id: prev.length + 1, type: 'system', text: bubble, completed: true }
      ]));
      // Persist system markers:
      // 1) button_selected for ServiceNow
      // 2) "servicenow catalog newtab" to indicate we showed the new-tab bubble
      // 3) "you can continue chatting here with workpal" and enable input
      try {
        const conversationId = hybridChatService.currentConversationId;
        const baseMeta = {
          timestamp: new Date().toISOString(),
          domain_id: RESOLVED_DOMAIN_ID,
          request_id: info.requestId || `REQ-${crypto?.randomUUID?.() || Math.random().toString(36).slice(2)}`
        };
        // 1) selection
        {
          const chatId = `system_${Date.now()}_${Math.random().toString(36).slice(2, 14)}`;
          const content = `<<["system","button_selected","ServiceNowCatalog"]>>`;
          await conversationStorage.addMessage(conversationId, 'system', content, baseMeta, [], chatId);
        }
        // 2) new-tab bubble marker
        {
          const chatId = `system_${Date.now()}_${Math.random().toString(36).slice(2, 14)}`;
          const content = `<<["system","servicenow catalog newtab"]>>`;
          await conversationStorage.addMessage(conversationId, 'system', content, baseMeta, [], chatId);
        }
        // 3) WorkPal continue message + re-enable input
        {
          const chatId = `system_${Date.now()}_${Math.random().toString(36).slice(2, 14)}`;
        ...
        console.warn('⚠️ Failed to persist ServiceNow follow-up markers:', e?.message || e);
      }
      // Show a friendly WorkPal continuation bubble and re-enable input
      const workpalBubble = (
        <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-transparent text-white' : 'bg-transparent text-[#2c3e50]'}`} style={{ textAlign: 'left' }}>
          Hey — you can continue chatting here with <strong>WorkPal</strong>. Ask your next question anytime.
        </div>
      );
      setMessages(prev => ([
        ...prev,
        { id: prev.length + 1, type: 'system', text: workpalBubble, completed: true }
      ]));
      setIsLiveAgentCardShowing(false);
      return;
    }
    ...
    if (info.type === 'continue') {
      ...
      const bubble = (
        <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-transparent text-white' : 'bg-[#f8f9fa] text-[#2c3e50]'}`} style={{ textAlign: 'left' }}>
          {info.message || 'You can continue chatting here in WorkPal.'}
        </div>
      );
      setMessages(prev => ([...prev, { id: prev.length + 1, type: 'system', text: bubble, completed: true }]));
      // Persist a system marker so retrieval renders styled banner later
      try {
        // Persist a system marker indicating the user selected Continue Chatting (button_selected)
        try {
          const conversationId = hybridChatService.currentConversationId;
          const chatId = `system_${Date.now()}_${Math.random().toString(36).slice(2, 14)}`;
          ...
          await conversationStorage.addMessage(conversationId, 'system', content, metadata, [], chatId);
        } catch (e) {
          console.warn('⚠️ Failed to persist ContinueChatting system marker:', e?.message || e);
        }
      } catch (e) { console.warn('Failed to store continue-chatting system marker:', e); }
  ...
  ws.onmessage = async (event) => {
        try {
          ...
          // Some servers wrap the real payload as a JSON string in msg.text with raw:true
          // If so, parse the inner payload to access structured body fields
          try {
            if (msg?.raw === true && typeof msg?.text === 'string') {
          ...
          // Ignore top-level msg.text; rely exclusively on structured body entries
          // OutputText: separate agent vs system outputs to avoid duplicates
          const outputItems = Array.isArray(msg?.body)
            ? msg.body.filter(b => b.uiType === 'OutputText' && typeof b.value === 'string' && b.value.trim().length > 0)
            : [];
          const agentOutputItems = outputItems.filter(b => b.agentInfo && b.agentInfo.sentFromAgent === true);
          const systemOutputItems = outputItems.filter(b => !(b.agentInfo && b.agentInfo.sentFromAgent === true));
          const fromBody = agentOutputItems.length > 0 ? agentOutputItems[0].value.trim() : '';
          // Render only non-agent OutputText items as system bubbles
          if (systemOutputItems.length > 0) {
            let isDark = false;
            try {
              isDark = document.body.classList.contains('dark') ||
                       document.documentElement.classList.contains('dark') ||
            ...
            setMessages(prev => ([
              ...prev,
              ...systemOutputItems.map(item => ({
                id: prev.length + 1,
                type: 'system',
                text: (<div style={sysTextStyle}>{item.value.trim()}</div>)
              }))
            ]));
            try {
              const conversationId = hybridChatService.getCurrentConversationId();
              const baseMeta = {
                timestamp: new Date().toISOString(),
                domain_id: RESOLVED_DOMAIN_ID,
                request_id: requestId || `REQ-${crypto?.randomUUID?.() || Math.random().toString(36).slice(2)}`
              };
              for (const item of systemOutputItems) {
                const chatId = `system_${Date.now()}_${Math.random().toString(36).slice(2, 14)}`;
          ...
          // Find wait time message from structured body items
          const waitTimeMsg = Array.isArray(msg?.body)
            ? msg.body.find(b => b.uiType === 'ActionMsg' && b.actionType === 'StartSpinner' && b.spinnerType === 'wait_time' && b.waitTime)
            : undefined;

          const isWaitTimeEvent = !!(waitTimeMsg && waitTimeMsg.waitTime);
          if (isWaitTimeEvent) {
            // Render styled wait time bubble; adapt to dark/light themes
            let isDark = false;
            try {
              isDark = document.body.classList.contains('dark') ||
                       document.documentElement.classList.contains('dark') ||
            ...
            // Parse seconds (e.g., "16 Seconds" -> 16)
            const raw = String(waitTimeMsg.waitTime);
            const secondsMatch = raw.match(/(\d+)\s*/);
            const initialSeconds = secondsMatch ? parseInt(secondsMatch[1], 10) : 0;

            let createdId;
            setMessages(prev => {
              createdId = prev.length + 1;
              waitCountdownMsgIdRef.current = createdId;
              waitRemainingRef.current = initialSeconds;
              return ([
                ...prev,
                {
                  id: createdId,
                  ...
                  text: (
                    <div style={bubbleStyles}>
                      Connecting to a live agent. Estimated wait time: {initialSeconds} seconds
                    </div>
                  )
                }
              ]);
            });
            // Do not persist the wait time spinner bubble per requirement

            // Start countdown that updates the bubble every second
            if (waitCountdownRef.current) {
              clearInterval(waitCountdownRef.current);
            }
            if (initialSeconds > 0) {
              ...
              waitRemainingRef.current = waitRemainingRef.current - 1;
              const remaining = waitRemainingRef.current;
              setMessages(prev => prev.map(m => {
                if (m.id === waitCountdownMsgIdRef.current) {
                  return {
                    ...m,
                    text: (
                      <div style={bubbleStyles}>
                        Connecting to a live agent. Estimated wait time: {remaining} seconds
                      </div>
                    )
                  };
                }
          ...
          // Render ActionMsg System messages (e.g., "Agent has joined") as styled system bubbles
          const systemMsgs = Array.isArray(msg?.body)
            ? msg.body.filter(b => b.uiType === 'ActionMsg' && b.actionType === 'System' && typeof b.message === 'string' && b.message.trim().length > 0)
            : [];
          if (systemMsgs.length > 0) {
            ...
            // Stop countdown if active when a system message arrives
            if (waitCountdownRef.current) {
              clearInterval(waitCountdownRef.current);
              waitCountdownRef.current = null;
              const countdownId = waitCountdownMsgIdRef.current;
              setMessages(prev => prev.filter(m => m.id !== countdownId));
              waitCountdownMsgIdRef.current = null;
            ...
            setMessages(prev => ([
              ...
              ...systemMsgs.map(s => ({
                id: prev.length + 1,
                type: 'system',
                text: (<div style={sysStyle}>{s.message.trim()}</div>)
              }))
            ]));
            try {
              const conversationId = hybridChatService.getCurrentConversationId();
            ...
            // Try to capture agent name from join messages (e.g., "G, Haritha has joined.")
            try {
              const join = systemMsgs.find(s => /has joined\.?$/i.test(s.message || ''));
              if (join) {
                const m = String(join.message).replace(/\s+has joined\.?$/i, '').trim();
                if (m && m.length > 0) agentNameRef.current = m;
              }
            } catch (_) {}
          }
          const completed = msg?.completed === true;
          // Prefer agent name from body agentInfo, if present
          let agentName = msg?.agentName;
          try {
            if (Array.isArray(msg?.body)) {
              const info = msg.body.find(b => b.agentInfo && typeof b.agentInfo.agentName === 'string');
              if (info) agentName = info.agentInfo.agentName;
            }
          } catch (_) {}
          if (agentName && agentName.length > 0) {
            agentNameRef.current = agentName;
          }
          // Only use agent-derived text for live agent bubble to avoid duplicates
          const text = fromBody || '';
          ...
          // Stop countdown once a non-wait_time message arrives, and remove the bubble
          if (!isWaitTimeEvent && waitCountdownRef.current) {
            clearInterval(waitCountdownRef.current);
            waitCountdownRef.current = null;
            const countdownId = waitCountdownMsgIdRef.current;
            setMessages(prev => prev.filter(m => m.id !== countdownId));
          ...
          // Normal agent message (skip empty text to avoid blank bubbles)
          if (text && text.trim().length > 0) {
            const now = Date.now();
            const idleGap = now - (lastAgentMsgAtRef.current || 0);
            const showName = isFirstMessage() || idleGap > (5 * 60 * 1000);
            setMessages(prev => ([...prev, {
              id: prev.length + 1,
              type: 'live_agent',
              text: renderLiveAgentMessageUtil(text, showName, agentNameRef.current || agentName, isDarkMode)
            }]));
            try {
              const conversationId = hybridChatService.getCurrentConversationId();
              const baseMeta = {
              ...
              const chatId = `liveagent_${Date.now()}_${Math.random().toString(36).slice(2, 14)}`;
              const content = String(text).trim();
              if (content.length > 0) {
                await conversationStorage.addMessage(conversationId, 'liveagent', content, baseMeta, [], chatId);
              }
            } catch (_) {}
            lastAgentMsgAtRef.current = now;
          }
        } catch (err) {
  ...
  const postSystemAndTerminate = (message, domainid) => {
    // Persist system reason and styled marker before terminating
    (async () => {
      try {
        const conversationId = hybridChatService.getCurrentConversationId();
        const requestId = requestIdRef.current || `REQ-${crypto?.randomUUID?.() || Math.random().toString(36).slice(2)}`;
        ...
        // Build both a plain system message and a marker-based message for consistent styling on reload
        const systemMsgs = [
          { message },
        ...
        console.warn('⚠️ Failed to persist system termination messages:', persistErr);
      }
    })();
  ...
  const endChat = async () => {
    setShowConfirm(false);
    // Persist system end_conversation marker before terminating
    try {
      const conversationId = hybridChatService.getCurrentConversationId();
      const requestId = requestIdRef.current || `REQ-${crypto?.randomUUID?.() || Math.random().toString(36).slice(2)}`;
      const baseMeta = {
      ...
      const systemMsgs = [
        { message: `<<["system","end_conversation","You have ended the conversation"]>>` }
      ];
      for (const s of systemMsgs) {
        const chatId = `system_${Date.now()}_${Math.random().toString(36).slice(2, 14)}`;
        const content = String(s.message).trim();
      ...
      console.warn('⚠️ Failed to persist end_conversation marker:', persistErr);
    }
  ...
  // Countdown for wait_time spinner
  const waitCountdownRef = useRef(null);
  const waitCountdownMsgIdRef = useRef(null);
  const waitRemainingRef = useRef(0);
  const suppressAutoScrollRef = useRef(false); // Prevent auto-scroll to bottom during pagination prepend
  const atBottomRef = useRef(true); // Tracks whether user is near the bottom
  const forceScrollRef = useRef(false); // Force scroll to bottom on explicit actions (user send)
  ...
  useEffect(() => {
    // Only auto-scroll to bottom when not in the middle of a pagination prepend
    if (!suppressAutoScrollRef.current) {
      // Scroll if user is already near bottom or we explicitly requested it
      if (forceScrollRef.current || atBottomRef.current) {
        scrollToBottom();
        forceScrollRef.current = false; // consume the force flag
      }
    }
  }, [messages]);

  // Mount a sticky live-agent banner at top-left of chat window using a lightweight DOM element
  useEffect(() => {
    try {
      const existing = document.getElementById('live-agent-sticky-banner');
      if (liveAgent) {
        const name = agentNameRef.current || '';
        const el = existing || document.createElement('div');
        el.id = 'live-agent-sticky-banner';
        el.style.position = 'fixed';
        el.style.top = '12px';
        // Nudge right so it doesn't overlap the left sidebar
        el.style.left = '330px';
        el.style.zIndex = '1000';
        el.style.padding = '6px 10px';
        el.style.borderRadius = '12px';
        el.style.background = 'rgba(18, 47, 101, 0.08)';
        el.style.color = isDarkMode ? '#A0BEEA' : '#1a366f';
        el.style.fontWeight = '700';
        el.style.fontSize = '12px';
        el.style.boxShadow = '0 1px 4px rgba(0,0,0,0.08)';
        el.style.backdropFilter = 'blur(2px)';
        // Compose inner content with an active dot before text
        const dotColor = isDarkMode ? '#FFFFFF' : '#1a366f';
        el.innerHTML = `
          <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${dotColor};margin-right:8px;"></span>
          <span>Live agent${name ? `: ${name}` : ''}</span>
        `;
  ...
  }, [liveAgent, isDarkMode, agentNameRef.current]);
  ...
  // Pagination config and refs
  const PAGE_SIZE = 10; // Show 10 messages initially, then load 10 more on demand
  const fullMessagesRef = useRef([]); // Holds the full message list for the active conversation (UI format)
  const chatScrollRef = useRef(null); // Scroll container for preserving position on prepend and detecting scroll top
  const scrollHandlerAttachedRef = useRef(false);
  const scrollTriggerInProgressRef = useRef(false);
    ...
    const loadConversationFromUrl = async () => {
      ...
      try {
        ...
        const conversation = await conversationLoader.loadConversation(urlConversationId, {
          ...
          // Note: backend currently returns full conversation; we'll paginate locally
          messageOffset: 0,
          messageLimit: 50,
        ...
        if (conversation && conversation.messages) {
          ...
          const uiMessages = conversation.messages.map((msg, index) => {
            ...
            // Preserve original message_type to support user/system/liveagent rendering
            const normalizedType = (() => {
              const mt = (msg.message_type || '').toLowerCase();
              if (mt === 'user' || mt === 'assistant' || mt === 'system' || mt === 'liveagent') return mt;
              // Fallback: non-user becomes assistant to avoid breaking old threads
              return mt === 'user' ? 'user' : 'assistant';
            })();
            return {
              id: index + 1,
              type: normalizedType,
          ...
          // Store full list for pagination and show only the latest PAGE_SIZE
          fullMessagesRef.current = uiMessages;
          const initialSlice = uiMessages.slice(-PAGE_SIZE);
          setMessages(initialSlice);
          setApiTriggered(true);

          // Check if there are more messages to load
          const hasMore = (Array.isArray(uiMessages) ? uiMessages.length : 0) > PAGE_SIZE;
  ...
  suppressAutoScrollRef.current = true;
      ...
      const full = Array.isArray(fullMessagesRef.current) ? fullMessagesRef.current : [];
      const visibleCount = messages.length;
      const fullCount = full.length;
      const nextCount = PAGE_SIZE;

      // Compute slice to prepend: the chunk just before the earliest visible message
      const endIndex = Math.max(fullCount - visibleCount, 0);
      const startIndex = Math.max(endIndex - nextCount, 0);
      const prependSlice = full.slice(startIndex, endIndex);

      if (prependSlice.length > 0) {
        setMessages(prev => [...prependSlice, ...prev]);

        // Update hasMore based on whether we've reached the beginning
        const newVisible = visibleCount + prependSlice.length;
        setHasMoreMessages(fullCount > newVisible);
        ...
        setTimeout(() => {
          ...
          // Re-enable auto-scroll for future events
      ...
      console.error('❌ Failed to load more messages (local paginate):', error);
      setHasMoreMessages(false);
      suppressAutoScrollRef.current = false;
    } finally {
  ...
  // Attach scroll handler once to enable infinite scroll when near the top
  useEffect(() => {
    const container = chatScrollRef.current;
    if (!container || scrollHandlerAttachedRef.current) return;
    const THRESHOLD_PX = 80; // distance from top to trigger
    const BOTTOM_THRESHOLD_PX = 40; // distance from bottom to consider "at bottom"

    const onScroll = () => {
      try {
        // Track bottom proximity to decide if we should auto-scroll on new messages
        const distanceFromBottom = (container.scrollHeight - container.scrollTop - container.clientHeight);
        atBottomRef.current = distanceFromBottom <= BOTTOM_THRESHOLD_PX;

        if (!hasMoreMessages) return;
        if (loadingMoreMessages) return;
        if (scrollTriggerInProgressRef.current) return;
        if (container.scrollTop <= THRESHOLD_PX) {
          // Prevent re-entry until current load finishes
          scrollTriggerInProgressRef.current = true;
          // Use existing loadMoreMessages, which preserves scroll
          Promise.resolve(loadMoreMessages()).finally(() => {
            scrollTriggerInProgressRef.current = false;
          });
        }
      } catch (_) {}
    };
  ...
  useEffect(() => {
    try {
      ...
      const hasLocal = local && Array.isArray(local.messages) && local.messages.length > 0;
      if (hasLocal) {
        // Store full local (may be partial if backend only provided top-N) and show latest PAGE_SIZE
        fullMessagesRef.current = local.messages;
        const latest = local.messages.slice(-PAGE_SIZE);
        setMessages(latest);
        setHasMoreMessages(local.messages.length > latest.length);
        setLoadingMoreMessages(false);
      } else {
        // No local; clear current view while we hydrate
      ...
      (async () => {
        try {
          ...
          // Convert API messages to UI format (preserve IDs based on global index)
          const ui = convo.messages.map((msg, index) => {
            const mt = (msg.message_type || '').toLowerCase();
            const normalizedType = (mt === 'user' || mt === 'assistant' || mt === 'system' || mt === 'liveagent') ? mt : (mt === 'user' ? 'user' : 'assistant');
            return {
              id: index + 1,
              type: normalizedType,
              text: msg.content,
          ...
          const stillSame = (currentThread?.id || hybridChatService.getCurrentConversationId()) === convId;
          if (stillSame) {
            fullMessagesRef.current = ui;
            // If we currently show fewer than PAGE_SIZE or nothing, update view to last PAGE_SIZE
            setHasMoreMessages(ui.length > Math.max(messages.length, PAGE_SIZE));
            if (messages.length === 0 || messages.length <= PAGE_SIZE) {
              const latest = ui.slice(-PAGE_SIZE);
              setMessages(latest);
            }
          }
        } catch (e) {
          // Non-fatal; keep local view
        }
      })();
    } catch (_) {}
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // Keep fullMessagesRef in sync when we append new messages to the visible list (user send/assistant response)
  useEffect(() => {
    try {
      const full = Array.isArray(fullMessagesRef.current) ? fullMessagesRef.current : [];
      const visible = Array.isArray(messages) ? messages : [];
      if (visible.length === 0) return;
      // If visible extends the full list with new tail, append the missing ones
      if (visible.length >= full.length && full.length > 0) {
        const fullIds = full.map(m => m.id);
        const visIds = visible.map(m => m.id);
        const prefixMatches = fullIds.every((id, idx) => id === visIds[idx]);
        if (prefixMatches) {
          const newTail = visible.slice(full.length);
          if (newTail.length > 0) {
            fullMessagesRef.current = [...full, ...newTail];
          }
          return;
        }
      }
      // If we don't have a full list yet, initialize it with visible
      if (full.length === 0) {
        fullMessagesRef.current = [...visible];
      }
    } catch (_) {}
  }, [messages]);
  ...
  <div className={`flex-1 overflow-y-auto p-5`} ref={chatScrollRef}>
          <div 
            className="mx-auto"
            style={{
              display: 'flex',
          ...
          >
            ...
            {/* Infinite scroll: load older messages automatically when near the top */}
            {loadingMoreMessages && (
              <div className="w-full flex justify-center mt-1">
                <div className={`flex items-center gap-2 text-xs ${isDarkMode ? 'text-[#A0BEEA]' : 'text-[#2861BB]'}`}>
            ...
            {messages.map((message) => {
              ...
              <div key={message.id} style={{ width: '100%' }}>
                {message.type === 'user' ? (
                  ...
                  <div className="space-y-4">
                    {/* Marker-specific components are rendered via formatTextWithLinks downstream; no additional generic bubble here */}
                    {/* Persisted system markers: render ButtonRow replica with selected and disabled + instructions for previous threads */}
                    {typeof message.text === 'string' && /<<\s*\[\s*"system"\s*,\s*"button_selected"/i.test(message.text) ? (
                      (() => {
                        // Extract selection label from marker: <<["system","button_selected","<Label>"]>>
                        const match = message.text.match(/<<\s*\[\s*"system"\s*,\s*"button_selected"\s*,\s*"([^"]+)"\s*\]\s*>>/i);
                        const selectedLabel = match ? match[1] : '';
                        const options = [
                          { key: 'AgenticHRAdvisor', label: 'Manager coaching and coaching for corrective action' },
                          { key: 'AgenticContactCenter', label: 'Other HR support' },
                          { key: 'ServiceNowCatalog', label: 'ServiceNow ticket catalog' },
                          { key: 'ContinueChatting', label: 'Continue chatting' }
                        ];
                        const isSelected = (opt) => (selectedLabel && (selectedLabel === opt.key || selectedLabel === opt.label));
                        return (
                           <div style={{
                            ...
                            <div className="grid grid-cols-1 gap-3">
                              {options.map((opt, idx) => (
                                <button
                                  key={`${opt.key}-${idx}`}
                                  disabled
                                  className={`w-full text-left px-4 py-3 rounded-lg border ${isSelected(opt)
                                    ? (isDarkMode ? 'bg-[#1F3E81] border-[#2861BB] text-white' : 'bg-white border-[#2861BB] text-[#1a366f]')
                                    : (isDarkMode ? 'bg-transparent border-[#1F3E81] text-[#A0BEEA]' : 'bg-[#f3f5f7] border-[#e3e8ef] text-[#6b7a90]')
                                  } cursor-not-allowed`}
                                  title={opt.label}
                                >
                                  {opt.label}
                    ...
                    {message.isLiveAgentCard ? (
                      <div style={{
                        ...
                        {/* Live card instructions are rendered here to ensure they always appear above the interactive buttons */}
                        <p style={{ fontSize: "14px", fontWeight: 600, marginBottom: "18px", textAlign: "left", letterSpacing: "0.2px" }}>
                          <span style={{ display: 'block', marginBottom: '12px' }}>
                            If you have a question related to manager coaching or corrective actions and wish to connect with a live agent, please click on <strong>"Manager coaching and coaching for corrective action"</strong>.
                          </span>
                        ...
                        <ButtonRow
                          ...
                          onRoute={onButtonRowRoute}
                          showInstructions={false}
                        />
                      </div>
                    ) : typeof message.text === 'string' ? (
                      // Use existing marker parser/formatter to render system messages consistently
              ...
              <div style={{ textAlign: 'center', marginBottom: '8px' }}>
                <button
                  ...
                  style={{
                    background: 'transparent',
                    border: 'none',
                    color: isDarkMode ? '#A0BEEA' : '#1a3673',
                    cursor: 'pointer',
                    fontSize: '16px',
                    fontWeight: 600,
                    textDecoration: 'none'
              ...
              <div style={{ textAlign: 'center', marginBottom: '8px' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '4px',
                    marginRight: '10px',
                    borderBottom: `1px solid ${isDarkMode ? '#A0BEEA' : '#1a3673'}`
                  }}
                >
                  Yes
                </button>
