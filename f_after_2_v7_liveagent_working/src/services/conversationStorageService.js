...
export class ConversationStorageService {
  ...
  async addMessage(conversationId, messageType, content, metadata = {}, referenceLinks = [], chatId = null) {
    try {
      ...
      // Verbose instrumentation: show exact backend URL and payload shape used to persist messages
      try {
        console.group('[Persist] ➡️ POST conversation message');
        console.log('URL:', url);
        ...
        console.log('reference_links:', messageData.reference_links);
        // Log a safe preview of content to avoid flooding the console
        const preview = (messageData.content || '').toString();
        console.log('content_preview:', preview.length > 500 ? preview.slice(0, 500) + '…' : preview);
        console.groupEnd();
      } catch (_) {
        // Fallback single-line payload log
