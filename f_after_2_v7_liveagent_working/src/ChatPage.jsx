  ...
  try {
    ...
    if (sysMarkerMatch) {
      ...
      if (marker === 'connecting_to_live_agent') {
        // Light/dark adaptive: transparent background, no borders
        let isDark = false;
        try {
        ...
        const colorClass = isDark ? 'text-white' : 'text-[#065a82]';
        return (
          <div>
            <style>{`
              ...
              .force-blue-text, .force-blue-text * {
                ...
                -webkit-text-fill-color: #065a82 !important;
              }
            `}</style>
            <div
              className={`system-banner connecting-to-agent flex items-center gap-3 rounded-[10px] px-1 py-2 text-sm leading-snug ${colorClass} ${isDark ? 'force-white-text' : 'force-blue-text'}`}
            >
            <img src={PulseIconSvg} alt="Pulse" className="h-5 w-5" />
            <span><strong className="font-semibold">Connecting you to a live HR advisor…</strong> Please wait while we establish the session.</span>
            </div>
          </div>
        );
      }
      ...
      if (marker === 'end_conversation' || marker === 'session_ended') {
        return (
          <div className="system-banner end-conversation flex items-center gap-3 rounded-[10px] border border-red-300/60 bg-red-100/40 px-3 py-2 text-[#7a2b2b] text-sm leading-snug">
            <span className="font-bold">Live-agent session ended.</span>
            <span>You can continue chatting here or start a new conversation from the sidebar.</span>
          </div>
        );
      }
      ...
      try {
        setMessages(prev => ([
          ...prev,
          {
            id: prev.length + 1,
            type: 'system',
            text: renderLiveAgentMessage('Connecting you with an HR Advisor...', true, 'Agentic HR Advisor'),
            completed: true
          }
        ]));
      } catch (_) {}
      ...
      try {
        setMessages(prev => ([
          ...prev,
          {
            id: prev.length + 1,
            type: 'system',
            text: renderLiveAgentMessage('Connecting you with HR Support...', true, 'HR Support'),
            completed: true
          }
        ]));
      } catch (_) {}
        ...
        try {
          ...
          if (text && text.trim().length > 0) {
            ...
            setMessages(prev => ([...prev, {
              id: prev.length + 1,
              type: 'live_agent',
              text: renderLiveAgentMessage(text, showName, agentNameRef.current || agentName)
            }]));
            try {
              const conversationId = hybridChatService.getCurrentConversationId();
              const baseMeta = {
