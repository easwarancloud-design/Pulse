...
import { mockStreamingResponse } from './utils/mockWorkforceAgent';
...
const formatTextWithLinks = (text) => {
  ...
  const formattedTexQuote = formattedText.replace("'", "&apos;");
...
const ChatPage = ({ onBack, userQuestion, onToggleTheme, isDarkMode, currentThread, isNewChat, isNewChatActive, onNewChat, onThreadSelect, onFirstMessage }) => {
  ...
  const { getToken, useMock } = useAccessToken();
  ...
  const sendWorkforceAgentMessage = async (inputText, replaceExisting = false) => {
    ...
    try {
      ...
      // Check if we're in mock mode after token attempt
      if (useMock) {
        console.log('🎭 Using mock workforce agent responses');
        setMockMode(true);
        
        // Use mock streaming response
        for await (const chunk of mockStreamingResponse(inputText)) {
          if (chunk === '<<LiveAgent>>') {
      ...
      // Handle live agent routing for both real and mock
      if (liveAgentTriggered) {
        ...
        localChatHistory.saveChatEntry({
          ...
          chat_type: useMock ? 'mock_bot' : 'bot',
          feedback_score: 0,
      ...
      console.error('Error fetching workforce agent response:', error);
  ...
  return (
    <div className={`flex h-screen ${isDarkMode ? 'bg-gradient-to-br from-[#072056] to-[#000B23]' : 'bg-[#F9FAFB]'}`}>
      ...
      <div className={`flex-1 flex flex-col h-screen relative z-10 ${isDarkMode ? 'bg-gradient-to-br from-[#072056] to-[#000B23]' : ''}`}>
        {/* Mock Mode Indicator */}
        {(mockMode || useMock) && (
          <div className={`${isDarkMode ? 'bg-orange-900/80 text-orange-200' : 'bg-orange-100 text-orange-800'} px-4 py-2 text-sm text-center border-b`}>
            🎭 Running in Mock Mode - Real API not accessible. All responses are simulated.
          </div>
        )}
                  ...
                  <div>
                    <div className="space-y-4">
                      <div className={`${isDarkMode ? 'text-[#A0BEEA]' : 'text-[#787777]'} text-sm leading-relaxed text-left`} style={{ lineHeight: '1.6', textAlign: 'left' }}>
                        ...
                        {!message.completed && message.chat_id === streamingMessageId && (
                    ...
                    {message.type === 'assistant' &&
                     ...
                     (message.chat_id ? (message.chat_id !== streamingMessageId) : true) && (
