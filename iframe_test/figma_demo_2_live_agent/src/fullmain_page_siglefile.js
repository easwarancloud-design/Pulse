...
// Import RotateCw for the missing refresh button
import {
  Menu, ChevronDown, Search, Bell, Mic, LayoutGrid, Globe, Compass, Users, MapPin, Briefcase, ChevronRight, Check, RotateCw 
} from 'lucide-react';
  ...
  return (
    <div className="relative overflow-hidden" style={gradientStyle}>
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 relative z-10">
        {/* Gradient Blur Effect (Now relying more on the CSS style, keeping this for depth) */}
        ...
        {/* Search Input Bar - REFINED BORDER/RING FOR SHARP LOOK */}
        <div className="relative bg-[#1A2F5A] rounded-xl flex items-center p-1 mb-8 shadow-2xl transition-all ring-2 ring-blue-500 border border-transparent">
        ...
        {/* Quick Action Buttons and Refresh Icon */}
        <div className="flex items-start">
            {/* The missing Refresh Icon, styled like a small action button */}
            {/* Using the same refined button styling for consistency */}
            <button className="p-3 mr-4 mt-2 rounded-full text-blue-300 hover:text-white transition-colors bg-[#1A2F5A] border border-white/20 shadow-lg">
                <RotateCw className="w-5 h-5" />
            </button>
            <div className="flex flex-wrap gap-4 mb-4">
                <QuickActionButton text="Share the latest company updates on AI developments" icon={ShareIcon} />
                <QuickActionButton text="What required learning do I have?" icon={Briefcase} />
                <QuickActionButton text="Whose in the office today?" icon={Users} />
            </div>
        </div>
        
        <a href="#" className="text-sm text-blue-300 hover:text-blue-200 transition-colors flex items-center justify-end font-medium pt-2">
          Learn More About AI
...
const QuickActionButton = ({ text, icon: Icon }) => (
  // REFINED BORDER: Using a subtle white/light blue border to match the image's delicate outline
  <button className="flex items-center px-4 py-2 bg-[#1A2F5A] text-white/90 text-sm rounded-full border border-white/20 hover:bg-[#253E6D] transition-colors whitespace-nowrap">
...
const App = () => {
  return (
    ...
    <div className="font-sans min-h-screen bg-gray-50">
      ...
      {/* Development Mode Floating Banner - Replicated from the image */}
      <div className="fixed bottom-4 right-4 z-20">
        <div className="flex items-center bg-green-500 text-white p-3 rounded-full shadow-xl">
          <span className="text-sm font-medium pr-4">
            Enjoy Dev Mode for up to 3 days while waiting for admin review.
          </span>
          <button className="bg-white text-green-600 p-1 rounded-full hover:bg-gray-100 transition-colors">
