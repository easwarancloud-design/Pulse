...
const handleFormSubmission = (query, type = 'manual', additionalData = {}) => {
  ...
  form.method = 'POST';
  form.action = 'https://results.example.com/search'; // Replace with your actual results page URL
  ...
  // Add parent URL for back navigation
  const parentUrlInput = document.createElement('input');
  parentUrlInput.type = 'hidden';
  parentUrlInput.name = 'parentUrl';
  parentUrlInput.value = getParentUrl();
  form.appendChild(parentUrlInput);
...
const ChatPage = ({ onBack, userQuestion, onToggleTheme, isDarkMode, currentThread, isNewChat, isNewChatActive, onNewChat, onThreadSelect, onFirstMessage }) => {
  ...
  // Function to determine if a message should show a table
  const shouldShowTable = (messageText, isFirstResponse = false) => {
    if (!messageText) return false;
    
    const lowerText = messageText.toLowerCase();
    
    // Show table only for specific types of queries
    const tableKeywords = [
      'training',
      'cyber security',
      'completion',
    ...
    // Check if message contains table-worthy keywords
    const hasTableKeywords = tableKeywords.some(keyword => 
      lowerText.includes(keyword)
    );
    
    // Show table only if:
    // 1. It's the first response AND has relevant keywords
    // 2. OR it's specifically about training/completion data
    return isFirstResponse && hasTableKeywords;
  };
  ...
  const [messages, setMessages] = useState(() => {
    ...
    if (urlQuery && urlType === 'manual') {
      ...
      return [
        ...
        },
        {
          id: 2,
          type: 'assistant',
          text: generateResponse(urlQuery),
          showTable: shouldShowTable(urlQuery, true)
        }
      ];
    }
    ...
    if (currentThread?.conversation?.length === 1 && currentThread.conversation[0].type === 'user') {
      ...
      return [
        ...
        },
        {
          ...
          showTable: shouldShowTable(currentThread.conversation[0].text, true)
        }
      ];
    }
    ...
    // Priority 2: Load existing conversation from current thread (with multiple messages)
      ...
      return currentThread.conversation.map((msg, index) => ({
        ...
        type: msg.type,
        text: msg.text,
        showTable: false, // Never show tables for existing conversations
    ...
    // Priority 3: Coming from main page with a manual question (legacy support)
      ...
      return [
        ...
        },
        {
          ...
          showTable: shouldShowTable(effectiveQuestion, true)
        }
      ];
    } 
    ...
    // Priority 5: Default fallback
      ...
      const defaultQuestion = 'Who from my team  hasn\'t completed the Cyber Security Training?';
      return [
        {
          id: 1,
          type: 'user',
        ...
        },
        {
          ...
          type: 'assistant',
          text: 'Based on the latest records, this is currently where your team members stand in regards to the Do The Right Thing: Cyber Security Training 2025:',
          showTable: shouldShowTable(defaultQuestion, true)
        }
      ];
    }
  });
  ...
  const handleSendMessage = () => {
    if (userInput.trim()) {
      ...
      if (handleFormSubmission(userInput, 'manual', { 
      ...
      // Continue with normal flow if not in iframe
      const userMessage = {
      ...
      const assistantResponse = {
        ...
        showTable: shouldShowTable(userInput, messages.length === 1) // Show table only for first response if keywords match
      };
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {message.suggestions?.map((suggestion, index) => (
                        <button 
                          key={index} 
                          onClick={() => {
                            const query = `Tell me about ${suggestion.toLowerCase()}`;
                            // Try form submission for iframe breakout first
                            if (!handleFormSubmission(query, 'predefined', { suggestion: suggestion })) {
                              // Fallback to normal flow if not in iframe
                              setUserInput(query);
                            }
                          }}
