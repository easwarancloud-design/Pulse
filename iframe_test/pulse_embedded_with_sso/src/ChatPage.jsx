...
import { createThreadFromPostData, loadExistingConversation, isIframeBreakout } from './utils/postDataUtils';
...
const ChatPage = ({ 
  ...
  // State for URL parameter-driven conversations
  const [urlParamData, setUrlParamData] = useState(null);
  const [loading, setLoading] = useState(false);

  // Load conversation data from URL parameters on component mount
  useEffect(() => {
    const loadFromUrlParams = async () => {
      ...
      const iframeHost = params.get('iframeHost'); // New parameter for iframe host URL
      ...
      if (sessionId || search) {
        ...
        try {
          if (sessionId) {
            ...
            // Create conversation data from URL params directly
            const conversation = {
              id: threadId || 'temp-' + Date.now(),
              title: source === 'predefined' ? `Question: ${search}` : 
                     source === 'thread_selection' ? search :
                     `Search: ${search}`,
              messages: [
                { type: 'user', text: search, timestamp: new Date().toISOString() },
                { type: 'assistant', text: `I understand you're asking about: ${search}. Let me help you with that.`, timestamp: new Date().toISOString() }
              ]
            };
            
            // Determine back target based on iframe host or context
            let backTarget;
            if (context === 'iframe' && iframeHost) {
              // Use the actual iframe host URL (e.g., http://localhost:3003/)
              backTarget = iframeHost;
            } else if (context === 'iframe') {
              // Fallback to pulse page
              backTarget = '/pulse';
            } else {
              // Embedded context
              backTarget = '/pulseembedded';
            }
  ...
  const [showBackButton, setShowBackButton] = useState(true); // true | false | 'disabled'
  const [backNavigationUrl, setBackNavigationUrl] = useState('/');

  // Check navigation context on mount
  useEffect(() => {
    const checkNavigationContext = () => {
      // Check if we have URL parameter data (independent access)
      if (urlParamData) {
        setShowBackButton(true);
        setBackNavigationUrl(urlParamData.backTarget || '/');
        return;
      }
      
      // Check if user came from iframe breakout
      const fromIframe = isIframeBreakout();
      
      // Check if user accessed directly (no referrer or referrer is same page)
      const referrer = document.referrer;
      const isDirectAccess = !referrer || 
        referrer === window.location.href || 
        (!referrer.includes('/pulsemain') && !referrer.includes('localhost:3000') && !fromIframe);
      
      if (isDirectAccess && !fromIframe && !postData) {
        // Direct access - show disabled back button
        setShowBackButton('disabled');
        setBackNavigationUrl('/');
      } else {
        // Always redirect to localhost:3003 for iframe breakouts or pulse navigation
        setBackNavigationUrl('http://localhost:3003');
        setShowBackButton(true);
      }
    };

    checkNavigationContext();
  }, [postData, urlParamData]);

  // Custom back handler
  const handleBack = () => {
    if (urlParamData && urlParamData.backTarget) {
      // Navigate to the specific back target for URL param data
      console.log('Navigating back to:', urlParamData.backTarget);
      
      if (urlParamData.backTarget.startsWith('http')) {
        // External URL (like iframe host) - redirect
        window.location.href = urlParamData.backTarget;
      } else {
        // Internal React route - use navigate
        navigate(urlParamData.backTarget);
      }
    } else if (backNavigationUrl.startsWith('http')) {
  ...
  const [userInput, setUserInput] = useState(() => {
    // Priority 1: Pre-fill from URL parameter data
    if (urlParamData?.source === 'predefined' || urlParamData?.source === 'manual_search') {
      return urlParamData.searchQuery || '';
    }
    // Priority 2: Pre-fill from POST data (iframe breakout)
    if (postData?.type === 'predefined') {
      return postData.question;
    }
    // Priority 3: Pre-fill input if this is a new chat with a predefined question
  ...
  const [messages, setMessages] = useState(() => {
    ...
    console.log('ChatPage useState - urlParamData:', urlParamData);
    
    // Priority -1: Handle URL parameter data
    if (urlParamData?.conversation) {
      console.log('Priority -1: URL parameter conversation data');
      return urlParamData.conversation.messages.map((msg, index) => ({
        id: index + 1,
        type: msg.type,
        text: msg.text,
        showTable: msg.type === 'assistant' && index === 1,
        isWelcome: false
      }));
    }
    
    // Priority 0: Handle POST data from iframe breakout
    if (postData) {
      if (postData.type === 'manual') {
        console.log('Priority 0: POST data manual search');
        return [
          {
            id: 1,
            type: 'user',
          ...
          },
          {
            id: 2,
            type: 'assistant',
            text: generateResponse(postData.question),
            showTable: false
          }
        ];
      } else if (postData.type === 'thread' && postData.conversation) {
        console.log('Priority 0: POST data thread load');
        try {
          const conversation = JSON.parse(postData.conversation);
          ...
          console.error('Error parsing POST conversation data:', error);
        }
      } else if (postData.type === 'predefined') {
        console.log('Priority 0: POST data predefined question');
        // For predefined questions, show welcome message and pre-fill input
        return [
          {
            id: 1,
            type: 'assistant',
    ...
    // Priority 4: New chat - show welcome message
    else if (isNewChat) {
      console.log('Priority 4: New chat welcome');
      return [
        {
          id: 1,
          type: 'assistant',
  ...
  // Update component state when URL parameter data loads asynchronously
  useEffect(() => {
    if (urlParamData?.conversation) {
      console.log('Updating messages from urlParamData:', urlParamData);
      
      // Update messages
      const newMessages = urlParamData.conversation.messages.map((msg, index) => ({
        id: index + 1,
        type: msg.type,
        text: msg.text,
        showTable: msg.type === 'assistant' && index === 1,
      ...
      // Update user input for predefined/manual searches
      if (urlParamData.source === 'predefined' || urlParamData.source === 'manual_search') {
        setUserInput(urlParamData.searchQuery || '');
      }
    }
  }, [urlParamData]);
  ...
  // Handle POST data from iframe breakout
  useEffect(() => {
    if (postData && onPostDataProcessed) {
      console.log('Processing POST data:', postData);
      
      if (postData.type === 'thread') {
        // Load existing thread
        const thread = createThreadFromPostData(postData);
        if (thread) {
          onThreadSelect(thread);
        }
      } else if (postData.type === 'predefined') {
        // Handle predefined question - create proper new chat
        const questionTitle = postData.question.length > 50 ? 
          postData.question.substring(0, 50) + '...' : 
          postData.question;
        ...
        // Set the user input to the question
        setUserInput(postData.question);
        
        // Create the thread and set it as new chat active
        ...
        try {
          ...
          if (stored) {
            ...
            threadsData = { today: [], yesterday: [], lastWeek: [], last30Days: [] };
          }
        ...
        // Set flags for new chat with predefined question
        onFirstMessage(newThread);
      } else if (postData.type === 'manual') {
        // Handle manual search - show question and response immediately
        const newThread = createThreadFromPostData(postData);
        if (newThread) {
          onThreadSelect(newThread);
        }
      }
  ...
  }, [postData, onPostDataProcessed, onThreadSelect, onFirstMessage]);
  ...
  // Show loading state while fetching URL parameter data
  if (loading) {
    return (
      <div className={`flex h-screen items-center justify-center ${isDarkMode ? 'bg-gradient-to-br from-[#072056] to-[#000B23]' : 'bg-[#F9FAFB]'}`}>
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-blue-500 mx-auto mb-4"></div>
          <p className={`text-lg ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>Loading conversation...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`flex h-screen ${isDarkMode ? 'bg-gradient-to-br from-[#072056] to-[#000B23]' : 'bg-[#F9FAFB]'}`}>
      ...
      <MenuSidebar 
        onBack={showBackButton === true ? handleBack : null}
