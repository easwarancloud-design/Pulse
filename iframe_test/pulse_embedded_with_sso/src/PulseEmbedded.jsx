...
const PulseEmbedded = ({ user, authService }) => {
  ...
  // Get the iframe host URL if running in iframe
  const getIframeHostUrl = () => {
    try {
      ...
      // Cross-origin restriction - try to get from document.referrer
  ...
  // Array of 9 predefined questions (same as main page)
  const allQuestions = [
    // Set 1 (0-2)
  ...
  // Enhanced search handler - navigate directly to React result page
  const handleSearch = (queryText = null, threadId = null, source = 'manual_search') => {
    ...
    // Get iframe host URL if running in iframe
    const iframeHostUrl = isInIframe ? getIframeHostUrl() : null;
    ...
    const params = new URLSearchParams({
      ...
      source: source,
      threadId: threadId || '',
      context: isInIframe ? 'iframe' : 'embedded',
      currentPage: 'pulse',
      timestamp: new Date().toISOString()
    });

    // Add iframe host URL if available
    if (iframeHostUrl) {
      params.set('iframeHost', iframeHostUrl);
    }

    // Navigate to React result page
    const resultUrl = `/resultpage?${params.toString()}`;
    
    if (isInIframe) {
      // For iframe, navigate the top window
      window.top.location.href = `http://localhost:3000${resultUrl}`;
    } else {
      // For embedded, use React router navigation
      navigate(resultUrl);
    }
  ...
  return (
    ...
    >
      {/* Background Effects - exact same as main page */}
      <svg 
        className="absolute" 
        style={{
          width: '1437px',
      ...
      {/* Content - exact same layout as main page */}
      <div className="relative w-full" style={{ zIndex: 10 }}>
        ...
        <div className="flex flex-col items-start gap-2.5 w-full max-w-[1184px] mx-auto relative"
          ref={inputRef}
        >
          {/* Main search form - visible HTML form */}
          <form 
            onSubmit={(e) => {
              e.preventDefault();
              handleSearch();
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-0 px-1 flex-shrink-0">
                ...
                <button type="button" className="flex items-center justify-center p-2 hover:opacity-80 transition-opacity">
      ...
      {/* About Box Popup - exact same styling as main page */}
      {showAboutBox && (
        <div 
