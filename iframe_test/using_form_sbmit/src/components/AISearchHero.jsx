...
const AISearchHero = ({ onSearch }) => {
  ...
  const inputConvRef = useRef(null);
  ...
  // Helper: submit via form to break out of iframe using GET + hash (safe for CRA/dev)
  const submitViaFormTop = (payload) => {
    const inIframe = typeof window !== 'undefined' && window.top && window.top !== window.self;
    if (!inIframe) return false;
    try {
      // Persist context for the result page to consume
      if (window.top && window.top.sessionStorage) {
        window.top.sessionStorage.setItem('aiResultNavigationContext', JSON.stringify(payload));
      } else if (window.sessionStorage) {
        // Fallback: current context (may not be visible to top)
        window.sessionStorage.setItem('aiResultNavigationContext', JSON.stringify(payload));
      }
    } catch {}
    try {
      // Build hash params for cross-origin-safe transport
      const parentRef = typeof document !== 'undefined' ? document.referrer : '';
      const hash = new URLSearchParams({
        type: payload.type || '',
        question: payload.question || '',
        conversationId: payload.conversationId || '',
        // Prefer absolute parent URL when available so Back can return to the embedding page
        returnUrl: payload.returnUrl || parentRef || window.location.pathname || '/',
      }).toString();
      if (formRef.current) {
        formRef.current.method = 'get';
        formRef.current.target = '_top';
        formRef.current.action = `/resultpage#${hash}`;
  ...
  const handleSuggestionClick = (suggestion) => {
    if (typeof suggestion === 'string') {
      // For predefined question, immediately trigger navigation to result page
      ...
      // If embedded, submit form to break out of iframe; else SPA
      const submitted = submitViaFormTop(payload);
      if (!submitted && onSearch) {
        onSearch(suggestion, null, 'predefined');
      }
    } else {
      // Handle thread suggestions (existing behavior)
  ...
  return (
    <div 
      ...
      style={{
        ...
        padding: 'clamp(16px, 4vw, 32px) clamp(16px, 10vw, 128px)',
        display: 'flex',
    ...
    >
      {/* Hidden form to allow navigation to top window (break out of iframe) */}
      <form
        ref={formRef}
        action="/resultpage"
        method="get"
        ...
        style={{ display: 'none' }}
      >
        <input ref={inputTypeRef} type="hidden" name="type" />
        <input ref={inputQuestionRef} type="hidden" name="question" />
        <input ref={inputConvRef} type="hidden" name="conversationId" />
        <input ref={inputReturnRef} type="hidden" name="returnUrl" />
      </form>
      <svg 
        className="absolute" 
        style={{
