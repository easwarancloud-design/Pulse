...
import { getStaticThreadsData } from './utils/postDataUtils';
...
const PulseEmbedded = () => {
  ...
  // Calculate input position for iframe dropdown positioning
  ...
  // Update position when suggestions show and on window resize
  useEffect(() => {
    if (showSuggestions) {
      calculateInputPosition();
      // Recalculate on window resize for responsive behavior
      const handleResize = () => calculateInputPosition();
  ...
  const handleInputFocus = () => {
    ...
    // Calculate position after a small delay to ensure layout is settled
    setTimeout(calculateInputPosition, 10);
  };
  ...
  // Helper function to submit form data to break out of iframe
  const submitToResultPage = (data) => {
    setFormData({
      ...data,
      timestamp: Date.now(),
  ...
  // Handle suggestion click - optimized for iframe breakout via form POST
  const handleSuggestionClick = (suggestion) => {
    if (typeof suggestion === 'string') {
      ...
      // Submit form to break out of iframe
      submitToResultPage({
        action: 'search',
        type: 'predefined',
        question: suggestion,
        userQuestion: suggestion // Pre-fill input field
      });
    } else {
      ...
      // Submit form to break out of iframe with thread data
      submitToResultPage({
        action: 'loadThread',
        type: 'thread',
        threadId: suggestion.id,
        threadTitle: suggestion.title,
        threadData: JSON.stringify(suggestion),
        conversation: JSON.stringify(suggestion.conversation || [])
      });
    }
  };
  ...
  const handleSearch = () => {
    if (searchQuery.trim()) {
      // Submit form to break out of iframe for manual search
      submitToResultPage({
        action: 'search',
        type: 'manual',
        question: searchQuery.trim(),
        userQuestion: '' // Clear input since we'll show response immediately
      });
      setSearchQuery('');
    }
  };
  ...
  return (
    <div 
      className="relative w-full h-screen overflow-hidden pulse-embedded-container"
      style={{
    ...
    >
      {/* Background SVG - adjusted for iframe */}
      <svg 
        className="absolute" 
        style={{
          width: '100%',
      ...
      <div className="relative w-full" style={{ zIndex: 10 }}>
        <div className="flex justify-between items-center mb-6">
          <div 
            className="text-white pulse-embedded-title"
            style={{
        ...
        <div className="flex flex-col items-start gap-2.5 w-full max-w-[1184px] mx-auto relative">
          ...
          >
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <input
                ...
                className="bg-transparent border-none outline-none flex-1 min-w-0 pulse-embedded-input"
                style={{
                  fontFamily: 'Elevance Sans, -apple-system, Roboto, Helvetica, sans-serif',
          ...
          {showSuggestions && suggestions.length > 0 && (
            isInIframe ? (
              ...
              // Normal dropdown for non-iframe context
              <div 
        ...
        <div className="flex flex-col justify-center items-start gap-2 w-full max-w-[1184px] mx-auto mt-6 pulse-embedded-questions-container">
          <div className="flex justify-between items-center w-full pulse-embedded-questions">
            <button
            ...
            <div className="flex justify-between items-center gap-2 md:gap-4 flex-1 ml-2 md:ml-4">
              {getCurrentQuestions().map((question, index) => (
                <button
                  ...
                  className="flex items-center px-2 md:px-4 py-3 cursor-pointer hover:opacity-80 transition-opacity relative group flex-1 pulse-embedded-question-button"
                  title={question}
                  style={{
                    borderRadius: '100px',
                    border: '1px solid #44B8F3',
                ...
                >
                  <div 
                    className="text-white truncate w-full text-sm md:text-base pulse-embedded-button-text"
                    style={{
                      fontFamily: 'Elevance Sans, -apple-system, Roboto, Helvetica, sans-serif',
      ...
      {/* Hidden Form for iframe breakout */}
      <FormSubmitter data={formData} onSubmitted={() => setFormData(null)} />
    </div>
  );
};

// Form Submitter Component for iframe breakout
const FormSubmitter = ({ data, onSubmitted }) => {
  React.useEffect(() => {
    if (data) {
      // Store data in sessionStorage first
      sessionStorage.setItem('pulsePostData', JSON.stringify(data));
      
      // Create dynamic form with a workaround for React routing
      const form = document.createElement('form');
      form.method = 'GET'; // Change to GET since we're using sessionStorage
      form.action = '/resultpage';
      form.target = '_top'; // Break out of iframe
      form.style.display = 'none';

      // Add a marker to indicate this came from POST
      const markerInput = document.createElement('input');
      markerInput.type = 'hidden';
      markerInput.name = 'fromPost';
      markerInput.value = 'true';
      form.appendChild(markerInput);

      // Add timestamp to ensure navigation
