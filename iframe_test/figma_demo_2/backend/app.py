...
import uvicorn
import requests
import httpx  # Added for async streaming
...
# logger=setup_async_logging()  # Temporarily disabled
import logging
...
# Shared async HTTP client for upstream streaming reuse
httpx_async_client = None

# --------------------
# Config helpers (env overrides supported)
# --------------------
def get_redis_config():
    host = os.getenv("REDIS_HOST", "master.rediscluster.gywvad.use2.cache.amazonaws.com")
    port = int(os.getenv("REDIS_PORT", "6379"))
    password = os.getenv("REDIS_PASSWORD", "RedisCluster2025")
    ssl_enabled = os.getenv("REDIS_SSL", "true").lower() in ("1", "true", "yes")
    return host, port, password, ssl_enabled

def get_mysql_config():
    return {
        "host": os.getenv("MYSQL_HOST", "aamsql-apm1009705-00dev01.c3q2fsxl5yla.us-east-2.rds.amazonaws.com"),
        "user": os.getenv("MYSQL_USER", "SRC_INTHELP_SLVR_WRITE"),
        "password": os.getenv("MYSQL_PASSWORD", "S7vcCw96uY$o0f%W"),
        "database": os.getenv("MYSQL_DB", "aamsqlapm1009705dev"),
    }
...
async def startup_redis():
    ...
    redis_client_async = None
    print("⚠️ Redis disabled for testing")

@app.on_event("shutdown")
async def shutdown_event():
    if redis_client_async is not None:
            # Gracefully close async Redis if initialized
            try:
                if redis_client_async is not None:
                    await redis_client_async.close()
            except Exception as e:
                # Log and continue shutdown
                logger.info(f"Redis async close skipped or failed: {e}")
    # Gracefully close shared HTTP client if initialized
    global httpx_async_client
    try:
        if httpx_async_client is not None:
            await httpx_async_client.aclose()
    except Exception as e:
        logger.info(f"HTTPX async client close skipped or failed: {e}")
...
def health_mysql():
    """Simple MySQL connectivity check: SELECT 1."""
    try:
        conn = get_connection()
        try:
            with conn.cursor() as cur:
                ...
                one = cur.fetchone()
            return {"ok": True, "db": "mysql", "result": str(one)}
        finally:
            conn.close()
    except Exception as e:
        return {"ok": False, "db": "mysql", "error": str(e)}


@app.get("/health/redis")
async def health_redis():
    """Check Redis connectivity with short timeouts to avoid hanging the request."""
    status = {"ok": False, "db": "redis", "sync": None, "async": None}

    # Build config
    r_host, r_port, r_pass, r_ssl = get_redis_config()

    # Use short timeouts for sync ping to avoid blocking; run in executor with global timeout
    try:
        tmp_sync = redis.Redis(
            host=r_host,
            port=r_port,
            password=r_pass,
            ssl=r_ssl,
        ...
        try:
            pong = await asyncio.wait_for(loop.run_in_executor(None, tmp_sync.ping), timeout=3)
            status["sync"] = bool(pong)
        finally:
            try:
                tmp_sync.close()
            except Exception:
        ...
        status["sync"] = f"error: {e}"

    # Async ping with short timeout
    try:
        tmp_async = Redis(
            host=r_host,
            port=r_port,
        ...
        try:
            pong_async = await asyncio.wait_for(tmp_async.ping(), timeout=2)
            status["async"] = bool(pong_async)
        finally:
            try:
                await tmp_async.close()
            except Exception:
                pass
    except Exception as e:
        status["async"] = f"error: {e}"

    status["ok"] = (status["sync"] is True) or (status["async"] is True)
    return status
...
async def startup_event():
    global app_wk
    global httpx_async_client
    # Temporarily disabled for testing - uncomment when agent_workflow is configured
    # app_wk = agent_workflow.workflow.compile()
    app_wk = None
    print("⚠️ Agent workflow disabled for testing")
    # Initialize shared async HTTP client for upstream streaming
    try:
        httpx_async_client = httpx.AsyncClient(
            timeout=httpx.Timeout(connect=5.0, read=60.0),
            limits=httpx.Limits(max_connections=50, max_keepalive_connections=20),
            http2=True,
            headers={"Accept-Encoding": "identity"},  # avoid gzip buffering delays
            verify=False  # mirror existing requests.verify=False behavior
        )
    except Exception as e:
        logger.info(f"Failed to init HTTPX async client: {e}")
...
# Removed legacy stream_response_original to reduce duplication and confusion
...
async def stream_response(user_input: dict):
    """Improved streaming: async HTTP, early chunk flush, smaller incremental yields."""
    start_time = time.time()
    logger.info(f"Stream started at: {start_time}")
    t_phase = {"start": start_time}
    ...
    # Use shared async client for connection reuse
    client = httpx_async_client or httpx.AsyncClient(timeout=httpx.Timeout(connect=5, read=60), headers={"Accept-Encoding": "identity"})
    t_phase["workflow_start"] = time.time()
    for output in app_wk.stream(user_input):
        if "workflow_first" not in t_phase:
            t_phase["workflow_first"] = time.time()
            if isinstance(output, dict) and output.get("classify_task", {}).get("task") == "get_liveagent":
                logger.info("Detected Live Agent Task")
                yield "<<LiveAgent>>"
                return
            ...
            if isinstance(output, dict) and "generate_response" in output:
                ...
                try:
                    req_start = time.time()
                    # Stream using httpx for real incremental bytes
                    # Ensure Accept-Encoding: identity to minimize buffering if not present
                    req_headers = dict(headers or {})
                    req_headers.setdefault("Accept-Encoding", "identity")
                    async with client.stream("POST", purl, headers=req_headers, content=payload) as resp:
                        conn_latency = time.time() - req_start
                        logger.info(f"Upstream connect+headers latency: {conn_latency:.2f}s status={resp.status_code}")
                        t_phase["upstream_connected"] = time.time()
                        if resp.status_code != 200:
                            text_err = await resp.aread()
                            logger.info(f"Response error code={resp.status_code} body={text_err[:200]!r}")
                            yield "Could not fetch response. Please check with Admin."
                            return
                        ...
                        async for raw_chunk in resp.aiter_bytes():
                            if not raw_chunk:
                                continue
                            chunk = raw_chunk.decode(errors="ignore")
                            # Remove SSE id/data boilerplate progressively
                            chunk = re.sub(r"\nid:.*?\n\n", "", chunk, flags=re.DOTALL)
                            chunk = chunk.replace("data: ", "").replace("Content-Length: 0", "")

                            if not first_chunk_time:
                                first_chunk_time = time.time()
                                logger.info(f"First byte after start: {first_chunk_time - start_time:.2f}s")
                                t_phase["first_byte"] = first_chunk_time
                    ...
                    # Second phase: policy links if required
                    if pre_task == "get_hrpolicy":
                        logger.info("Fetching HR policy links…")
                        try:
                            ...
                            policy_resp = await client.post(
                                "https://api.horizon.elevancehealth.com/v2/document/searches",
                                headers=headers,
                                content=json.dumps({"prompt": cleaned_response}),
                            )
                            logger.info(f"HR policy API latency after main stream: {time.time()-pol_start:.2f}s status={policy_resp.status_code}")
                            t_phase["policy_done"] = time.time()
                            if policy_resp.status_code == 200:
                                output_list = []
                                for item in policy_resp.json():
                                    if "document" in item:
                                        tags = item["document"].get("tags", [])
                                        title = next((t["value"] for t in tags if t.get("name") == "title"), None)
                                        url = next((t["value"] for t in tags if t.get("name") == "url"), None)
                            ...
                            logger.info(f"Policy retrieval error: {e}")
                            yield "⚠️ Unable to fetch reference link."
                except Exception as e:
                    logger.critical(f"Error connecting to downstream system: {e}")
                    yield "Error in fetch response, Please try again."

    end_time = time.time()
    logger.info(f"Total end-to-end duration: {end_time - start_time:.2f}s")
    # Emit structured timing summary
    summary = {
        "workflow_init_ms": int((t_phase.get("workflow_start") - t_phase["start"])*1000) if "workflow_start" in t_phase else None,
        "workflow_first_output_ms": int((t_phase.get("workflow_first") - t_phase["start"])*1000) if "workflow_first" in t_phase else None,
        "connect_latency_ms": int((t_phase.get("upstream_connected") - t_phase.get("workflow_first", t_phase["start"]))*1000) if "upstream_connected" in t_phase else None,
        "time_to_first_byte_ms": int((t_phase.get("first_byte") - t_phase["start"])*1000) if "first_byte" in t_phase else None,
        "policy_phase_ms": int((t_phase.get("policy_done") - t_phase.get("first_byte", end_time))*1000) if "policy_done" in t_phase else None,
        "total_ms": int((end_time - t_phase["start"])*1000)
    }
    logger.info(f"TimingSummary {summary}")
    logger.info(f"Question:{user_input['query']}")
...
# Removed legacy stream_response_samewith60 to reduce duplication

# Removed legacy stream_response_with60 to reduce duplication
...
if __name__ == "__main__":    
   uvicorn.run("app:app", host="0.0.0.0", port=8000, reload=False, log_config=None)
