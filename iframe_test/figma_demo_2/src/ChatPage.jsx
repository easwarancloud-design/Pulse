...
const formatTextWithLinks = (text) => {
  ...
  if (isCompleted) {
    console.log('✅ Bold conversion completed. Tags created:', (formattedText.match(/<strong>/g) || []).length);
    console.log('🔍 Sample bold text in HTML:', formattedText.match(/<strong>[^<]*<\/strong>/g)?.slice(0, 3) || 'None found');
  }
...
const ChatPage = ({ onBack, userQuestion, onToggleTheme, isDarkMode, currentThread, isNewChat, isNewChatActive, onNewChat, onThreadSelect, onFirstMessage, userInfo }) => {
  ...
  const RESOLVED_DOMAIN_ID = (userInfo?.domainId || localStorage.getItem('domainid') || DEFAULT_DOMAIN_ID).toUpperCase();
  ...
  const domainid = RESOLVED_DOMAIN_ID; // Use Okta domain if present, else fallback
                  ...
                  <div style={{
                    ...
                    <ButtonRow 
                      ...
                      onAgentConnect={(connectionInfo) => {
                        ...
                        // Handle agent connection messages
  ...
  useEffect(() => {
    ...
    if (existing !== RESOLVED_DOMAIN_ID) {
      ...
      console.log('🔑 Synced domain ID in localStorage:', RESOLVED_DOMAIN_ID);
    }
  }, []);
