...
const MessageActionIcons = ({ messageId, onActionClick, isLoading }) => {
  ...
  const handleAction = (action) => {
    ...
    // Always call the parent onActionClick for all actions including reload
    if (onActionClick) {
  ...
  return (
    <>
      ...
      {/* Reload Button */}
      <button 
        className={`p-1 transition-colors hover:opacity-80 ${isLoading ? 'animate-spin' : ''} bg-red-500`}
        style={{ color: isLoading ? colors.PRIMARY_COLOR : iconColor, backgroundColor: 'red' }}
        title={isLoading ? "Regenerating..." : "Reload/Regenerate"}
        onClick={() => {
          window.alert('RELOAD CLICKED!');
...
const ChatBubble = ({ children, isUser, isStreaming, isRegenerating, messageId, onActionClick, messageLoadingStates }) => {
  const { colors } = useTheme();
      ...
      <div className="flex flex-col max-w-2xl w-full">
        ...
        >
          <div className="text-sm text-left">
            ...
            {isRegenerating && (
              <span className="text-xs opacity-60 ml-2">(Regenerating...)</span>
            )}
          </div>
        </div>
        ...
        {!isUser && (
          <div className="flex items-center space-x-2 mt-2 ml-1">
            <MessageActionIcons 
              ...
              isLoading={messageLoadingStates?.[messageId] || false}
            />
          </div>
        )}
      </div>
...
const DataTableComponent = ({ onActionClick, messageLoadingStates }) => {
  const { colors } = useTheme();
    ...
    <div className="w-full mt-4">
      ...
      <div className="flex items-center space-x-2 mt-4">
        <MessageActionIcons 
          messageId="3" 
          onActionClick={onActionClick} 
          isLoading={messageLoadingStates?.["3"] || false}
        />
      </div>
    </div>
  );
...
const MainContent = ({ userQuestion, currentChat, onUpdateChatTitle, refreshTrigger }) => {
  ...
  const handleActionClick = async (messageId, action) => {
    console.log(`🔄 Action ${action} clicked for message ${messageId}`);
    
    if (action === 'reload') {
      console.log('🚀 Starting reload process...');
      
      // Set domainid if not present
      if (!localStorage.getItem('domainid')) {
        localStorage.setItem('domainid', 'TESTUSER123');
        console.log('👤 Set default domainid: TESTUSER123');
      }
      
      // Set loading state for this specific message
      setMessageLoadingStates(prev => ({ ...prev, [messageId]: true }));
      
      // Find the original user question for this response
      const messageIndex = chatMessages.findIndex(msg => msg.id === messageId);
      let foundUserQuestion = userQuestion || currentChat?.title || "Who from my team hasn't completed the Cyber Security Training?";
      
      // Look for the previous user message
      for (let i = messageIndex - 1; i >= 0; i--) {
        if (chatMessages[i].isUser) {
          foundUserQuestion = chatMessages[i].text;
          break;
        }
      }
      
      console.log('📝 User question found:', foundUserQuestion);
      
      if (foundUserQuestion) {
        try {
          console.log('🔥 Calling regenerateResponse...');
          await regenerateResponse(messageId, foundUserQuestion);
          console.log('✅ Regenerate completed successfully');
        } catch (error) {
          ...
          setChatMessages(prev => 
            prev.map(msg => 
              msg.id === messageId 
                ? { ...msg, text: msg.text + " [Error: Failed to regenerate response. Please try again.]", error: true }
                : msg
            )
          );
        } finally {
          setMessageLoadingStates(prev => ({ ...prev, [messageId]: false }));
        }
      }
    } else {
      console.log(`ℹ️ Other action: ${action}`);
    }
  };

  // Token function (simplified version of the one from workforceagent_frontend.js)
  ...
  const regenerateResponse = async (messageId, userQuestion) => {
    try {
      console.log('🎯 regenerateResponse called with:', { messageId, userQuestion });
      const domainid = localStorage.getItem('domainid') || 'TESTUSER123';
      ...
      setChatMessages(prev => 
        prev.map(msg => 
          msg.id === messageId 
            ? { ...msg, text: "Regenerating response...", isStreaming: true, isRegenerating: true }
            : msg
        )
      );

      // For now, let's add a fallback that simulates API response
      // This helps test if the button clicking mechanism works
      ...
      try {
        ...
        while (true) {
          ...
          if (chunk.includes("<<LiveAgent>>")) {
            console.log('🤖 LiveAgent detected in regenerated response');
            setChatMessages(prev => 
              prev.map(msg => 
                msg.id === messageId 
                  ? { ...msg, text: "🔄 Connecting you to a live agent. Please wait...", isStreaming: false, isRegenerating: false }
                  : msg
              )
            );
            break;
        ...
        setChatMessages(prev => 
          prev.map(msg => 
            msg.id === messageId 
              ? { ...msg, isStreaming: false, isRegenerating: false }
                : msg
          )
        );
        ...
        // Fallback response
        const fallbackResponse = `[REGENERATED] This is a regenerated response for the question: "${userQuestion}". The original API was not accessible, but this demonstrates that the reload functionality is working correctly. The response would normally be fetched from the workforce agent API.`;
        
        setChatMessages(prev => 
          prev.map(msg => 
            msg.id === messageId 
              ? { ...msg, text: fallbackResponse, isStreaming: false, isRegenerating: false }
              : msg
          )
        );
      }
  ...
  const handleActionClick_OLD = (messageId, action) => {
    console.log(`Action ${action} clicked for message ${messageId}`);
