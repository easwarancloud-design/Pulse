...
def update_email(state: AgentState):
    ...
    effective_date = today - timedelta(days=offset)
    print('Effective date:', effective_date.strftime('%Y-%m-%d'))
    ...
    # prompt=f'''You are a friendly, helpful conversational agent for inquiries related to user email update or change request,Respond only from the given question and process,xml response. Here is the user question: {state["query"]}.email address provided:{state['email']}
    # Answer the question , if required tell them process like in chat we can update Personal email address , user just need to tell like "update my phone number <email address>" 
    ...
    prompt=f'''You are a friendly, helpful conversational agent for inquiries related to updating personal email addresses. Respond only based on the given user question and the provided XML response.

        User question: {state["query"]}
        Email address provided: {state['email']}
        API XML response:
        {api_response}

        Answer the user's question regarding updating their *personal* email address. Inform them that only personal email addresses can be updated through this process, not work email addresses. If the personal email address can be updated via chat, explain the process. For example, you can say: "To update your personal email address, simply tell me the new email address you'd like to use, like 'update my personal email address to <new email address>'."

        Note: Based *only* on the provided XML response, determine if the email update was successful or not. If there's a reason mentioned in the XML for failure, include that in your response. Do not mention the XML itself, the data source, or any references to it.
        '''
