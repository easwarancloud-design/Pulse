...
const formatTextWithLinks = (text) => {
  ...
  // Only log for completed messages to reduce console spam
  const isCompleted = !text.includes('🔄 Regenerating');
  
  if (isCompleted) {
    console.log('🎨 formatTextWithLinks processing completed message');
    console.log('🔍 ** patterns found:', text.match(/\*\*[^*\n]+?\*\*/g) || 'None');
  }
  
  // If text already has <strong> tags, don't process it again
  if (text.includes('<strong>')) {
    const formattedTexQuote = text.replace(/'/g, "&apos;");
...
const ChatPage = ({ onBack, userQuestion, onToggleTheme, isDarkMode, currentThread, isNewChat, isNewChatActive, onNewChat, onThreadSelect, onFirstMessage }) => {
  ...
  // Inactivity limit for live agent
  const INACTIVITY_LIMIT = 19 * 60 * 1000; // 19 minutes in ms
  let inactivityTimer = null;

  // WebSocket connection function for live agent
  const connectWebSocket = (id) => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        const agentNameFromMsg = agentInfo?.agentName || null;
        ...
        const isFirstLiveMessage = agentNameFromMsg && !firstLiveAgentMessageShown.current;

        if (isFirstLiveMessage) {
          setAgentName(agentNameFromMsg);
        ...
        if (outputText) {
          setMessages(prev => [
            ...prev,
            {
              ...
              agentName: agentNameFromMsg,
              isFirstMessage: isFirstLiveMessage
            }
          ]);
        } else {
  ...
  // Initialize useAgentSession hook
  const {
    requestId,
    liveAgent,
    chatEnded,
    ...
    terminateSession,
    socketRef
  } = useAgentSession(connectWebSocket, setMessages, DEFAULT_DOMAIN_ID);
  ...
  // Workforce agent sendMessage function
  const sendWorkforceAgentMessage = async (inputText, replaceExisting = false) => {
    ...
    console.log('📤 Sending question:', inputText);

    const question_text = inputText;
    ...
    setStreamingMessageId(botChatId);
    setLoading(true);
    ...
    try {
      ...
      // Use real API
      console.log('🌐 Using real workforce agent API');
      console.log('📝 Request details:', {
        url: 'https://workforceagent.elevancehealth.com/workforceagent/chat',
        token: token ? 'Token exists' : 'No token',
        question: inputText,
        domainid: DEFAULT_DOMAIN_ID.toUpperCase(),
        method: 'GET',
        headers: {
      ...
      // If response is short, it might be a simple message. Update existing assistant message instead of appending.
      if (responseText.length < 50) {
        const shortText = responseText.trim() || 'Empty response received from API';
        setMessages(prev => prev.map(msg =>
          msg.chat_id === botChatId
            ? { ...msg, type: 'assistant', text: shortText, completed: true }
            : msg
        ));
        return;
      }
      ...
      while (true) {
        ...
        console.log(`📦 Chunk ${chunkCount} (${chunk.length} chars):`, chunk.substring(0, 100) + (chunk.length > 100 ? '...' : ''));
      ...
      console.log('⏱️ Total response time:', (completionTime - apiStartTime).toFixed(2), 'ms');
      console.log('📊 Total chunks received:', chunkCount);
      console.log('📏 Final message length:', partialMessage.length, 'characters');
      ...
      if (liveAgentTriggered) {
        ...
        setMessages(prev => prev.map(msg => 
          msg.chat_id === botChatId 
            ? {
                ...
                text: (
                  <div style={{
                    backgroundColor: isDarkMode ? "#2d3748" : "#f9fbfc",
                    padding: "20px",
                    borderRadius: "12px",
                    marginBottom: "16px",
                    fontFamily: "Segoe UI, Elevance Sans, sans-serif",
                    color: isDarkMode ? "#e2e8f0" : "#1a366f",
                    ...
                    <ButtonRow 
                      ...
                      onAgentConnect={(connectionInfo) => {
                        console.log('Agent connection:', connectionInfo);
                        
                        if (connectionInfo.type === 'continue') {
                          setIsInputDisabled(false);
                          setMessages(prev => [...prev, {
                            id: prev.length + 1,
                            type: 'system',
                            text: connectionInfo.message
                          }]);
                        } else if (connectionInfo.type === 'connecting') {
                        ...
                        } else if (connectionInfo.type === 'transferred') {
                          setMessages(prev => [...prev, {
                            id: prev.length + 1,
                            type: 'system',
                            text: connectionInfo.message
                          }]);
                          setIsInputDisabled(false); // Enable input for live agent chat
                        } else if (connectionInfo.type === 'error') {
                          setMessages(prev => [...prev, {
                            id: prev.length + 1,
                            type: 'system',
                            text: connectionInfo.message
        ...
        localChatHistory.saveChatEntry({
          ...
          chat_type: 'bot',
          feedback_score: 0,
          title: question_text.length > 50 ? question_text.substring(0, 50) + '...' : question_text
        });
      }
      ...
      console.error('📊 Partial message length at error:', partialMessage.length);
      ...
      setMessages(prev => prev.map(msg =>
        msg.chat_id === botChatId
          ? {
              ...
              completed: true,
              text: partialMessage 
                ? `${cleanStreamText(partialMessage)}\n\n⚠️ Unable to fetch complete response.`
  ...
  const sendLiveAgentMessage = async (inputText) => {
    if (!inputText.trim()) return;
    ...
    const payload = {
      ...
      userId: DEFAULT_DOMAIN_ID || "AG04333",
    ...
    try {
      ...
      console.error("❌ Failed to send message to live agent:", error);
      setMessages(prev => [...prev, {
        id: prev.length + 1,
        type: 'system',
  ...
  // Legacy generateResponse function for fallback
  ...
  const [messages, setMessages] = useState(() => {
    ...
    console.log('ChatPage useState - effectiveQuestion:', effectiveQuestion);
    console.log('ChatPage useState - urlQuery:', urlQuery);
    console.log('ChatPage useState - urlType:', urlType);
    ...
    if (urlQuery && urlType === 'manual') {
      console.log('Priority 0: URL manual input detected');
      return [
        {
          id: 1,
          type: 'user',
    ...
    // Priority 4: New chat or predefined questions - show welcome message or prepare for input
      ...
      console.log('Priority 4: New chat welcome or predefined question');
      return [
        {
          id: 1,
          type: 'assistant',
  ...
  useEffect(() => {
    if (!localStorage.getItem('domainid')) {
      localStorage.setItem('domainid', DEFAULT_DOMAIN_ID);
      console.log('🔑 Set default domain ID in localStorage:', DEFAULT_DOMAIN_ID);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (shouldCallLiveAPI && effectiveQuestion && !apiTriggered) {
      ...
      console.log('🔄 Triggering live API for:', questionText);
  ...
  const handleSendMessage = async () => {
    if (userInput.trim() && !loading) {
      ...
      if (liveAgent && !chatEnded) {
        // Send to live agent
        await sendLiveAgentMessage(userInput);
      } else {
        // Use workforce agent for normal chat
        await sendWorkforceAgentMessage(userInput);
  ...
  const handleReload = async (messageId) => {
    console.log('🔄 Reload clicked for message:', messageId);
    ...
    try {
      ...
      if (!userQuestion) {
        console.log('No user question found for reload');
        return;
      }
      
      console.log('🚀 Regenerating response for question:', userQuestion);
      ...
      // Always try to get a fresh token for reload, ignore mock mode
      console.log('🔄 Attempting to get fresh token for reload...');
      const domainId = localStorage.getItem('domainid') || 'AG04333';
      console.log('🔑 Using domain ID:', domainId);
      
      let token;
      try {
        token = await getToken(domainId, true); // Force real API
        console.log('🔑 Token received successfully for reload');
      } catch (tokenError) {
      ...
      console.log('🚀 Making real API call for reload...');
      console.log('🌐 API URL:', "https://workforceagent.elevancehealth.com/workforceagent/chat");
      console.log('❓ Question:', userQuestion);
      console.log('🏷️ Domain ID:', domainId);
      ...
      console.log('✅ API response received, starting to stream...');
          ...
          >
            {messages.map((message) => (
              <div key={message.id} style={{ width: '100%' }}>
                {message.type === 'user' ? (
                ...
                ) : message.type === 'live_agent' ? (
                  <div>
                    <div 
                      style={{
                        backgroundColor: isDarkMode ? "#1e293b" : "#f4f7fc",
                        padding: "10px 14px",
                        borderLeft: `4px solid ${isDarkMode ? "#3b82f6" : "#1a366f"}`,
                        borderRadius: "6px",
                        color: isDarkMode ? "#e2e8f0" : "#212121",
                    ...
                    >
                      {message.isFirstMessage && message.agentName && (
                        <div
                          style={{
                            color: isDarkMode ? "#60a5fa" : "#1a366f",
                            fontWeight: "600",
                            fontSize: "15px",
                            marginBottom: "6px"
                          }}
                      ...
                      <div className={`${isDarkMode ? 'text-[#e2e8f0]' : 'text-[#212121]'}`}>
                        {message.text}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div 
                      style={{
                        backgroundColor: isDarkMode ? "#374151" : "#f0f4f8",
                        padding: "8px 12px",
                        borderRadius: "6px",
                        color: isDarkMode ? "#9ca3af" : "#1a3673",
                        fontWeight: "500",
                        fontFamily: "Elevance Sans",
                        marginBottom: "8px",
                        textAlign: "center"
          ...
          <div className={`absolute right-0 top-0 h-full w-80 ${
            ...
            <div className={`p-4 border-b ${
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => setShowReferenceLinks(false)}
                  className={`p-1 hover:${isDarkMode ? 'bg-[#1F3E81]' : 'bg-gray-100'} rounded`}
                >
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M8 16C12.4183 16 16 12.4183 16 8C16 3.58172 12.4183 0 8 0C3.58172 0 0 3.58172 0 8C0 12.4183 3.58172 16 8 16ZM5.70711 4.29289C5.31658 3.90237 4.68342 3.90237 4.29289 4.29289C3.90237 4.68342 3.90237 5.31658 4.29289 5.70711L6.58579 8L4.29289 10.2929C3.90237 10.6834 3.90237 11.3166 4.29289 11.7071C4.68342 12.0976 5.31658 12.0976 5.70711 11.7071L8 9.41421L10.2929 11.7071C10.6834 12.0976 11.3166 12.0976 11.7071 11.7071C12.0976 11.3166 12.0976 10.6834 11.7071 10.2929L9.41421 8L11.7071 5.70711C12.0976 5.31658 12.0976 4.68342 11.7071 4.29289C11.3166 3.90237 10.6834 3.90237 10.2929 4.29289L8 6.58579L5.70711 4.29289Z" 
                      fill={isDarkMode ? "#A0BEEA" : "#787777"}
                    />
                  </svg>
                </button>
              </div>
