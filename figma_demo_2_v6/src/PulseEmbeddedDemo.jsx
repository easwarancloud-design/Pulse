...
const PulseEmbeddedDemo = ({ userInfo, domainId }) => {
  ...
  }, [domainId, effectiveUserInfo?.domainId, effectiveUserInfo?.domain_id]);
  ...
  const isInIframe = () => {
    try {
      ...
      return true; // Cross-origin -> treat as iframe
    }
  };
    ...
    if (isInIframe() && window.parent) {
      try {
        ...
        window.parent.postMessage({
          ...
          data: {
            suggestions,
            position: { x: inputRect.left, y: inputRect.bottom, width: inputRect.width },
            iframeId: 'pulseIframe'
          }
        }, '*');
      } catch (error) {
  ...
  const hideDropdownInParent = () => {
    if (isInIframe() && window.parent) {
      try {
        window.parent.postMessage({ type: 'PULSE_HIDE_DROPDOWN' }, '*');
      } catch (error) {
        console.error('Cannot send message to parent:', error);
      }
    }
  ...
  const effectiveDomainId2 = resolveDomainId();
        ...
        hybridChatService.setUserId(effectiveDomainId2);
  ...
  const handleSuggestionClick = (suggestion) => {
    ...
    if (typeof suggestion === 'string') {
      ...
      navigateToResults(suggestion.title, suggestion.id, 'thread');
    }
  };
  ...
  const handleSearch = () => {
    ...
    if (handleFormSubmission(query, 'manual', { fromSearch: 'true' })) {
      setSearchQuery('');
      return;
    }
    navigateToResults(query, null, 'manual');
    setSearchQuery('');
  };
          ...
          {showSuggestions && suggestions.length > 0 && (
            <div 
              ...
              style={{
                ...
                width: '100%',
                maxWidth: '1184px',
                maxHeight: isInIframe() ? '180px' : '320px',
                borderRadius: '16px',
                border: '2px solid #2861BB',
                background: 'linear-gradient(115deg, #122F65 2.06%, #00123C 97.35%)',
                padding: '12px',
