...
import PulseMain from './PulseMain.jsx';
import ChatPage from './ChatPage.jsx'; // Light mode
import PulseEmbedded from './PulseEmbedded.jsx';
import PulseEmbeddedDemo from './PulseEmbeddedDemo.jsx';
import ChatIntegrationDemo from './components/ChatIntegrationDemo.jsx';
import { useTheme } from './context/ThemeContext.jsx';
import { hybridChatService } from './services/hybridChatService.js';
import { conversationCacheService } from './services/conversationCacheService.js';
import { localConversationManager } from './services/localConversationManager.js';
import './App.css';

import { oktaAuth } from './oktaConfig.js';
import PrivateRoute from './PrivateRoute.jsx';
import { Security, useOktaAuth } from '@okta/okta-react';
import { ensurePublicIdentity, ALL_PUBLIC } from './config/accessControl.js';
...
function AppContent() {
  ...
  // Disable New Chat when a manual question is auto-submitted to prevent duplicates
...
// Centralized domainId resolution
  ...
  React.useEffect(() => {
    ...
    if (location.pathname === '/resultpage') {
      ...
      // Persist parentUrl from URL param exactly when present and valid
      try {
        const parentUrlParam = params.get('parentUrl');
        if (parentUrlParam && parentUrlParam !== 'unknown' && parentUrlParam !== 'false') {
          localStorage.setItem('pulse_parent_url', parentUrlParam);
          // Optional: log for traceability
          // console.log('💾 Stored parentUrl in localStorage:', parentUrlParam);
        }
      } catch (_) {}
      ...
      if (query) {
        if (conversationId) {
          ...
          // Temporarily disable New Chat while auto-send is in flight
          ...
          // Remove only the 'query' param to avoid reuse; keep conversationId and doc context
          try {
  ...
  const navigateToMain = () => {
    ...
    const storedParentUrl = (() => {
      try { return localStorage.getItem('pulse_parent_url') || null; } catch { return null; }
    })();
    // Prefer URL param when present/valid, else fall back to stored value
    const resolvedParentUrl = (parentUrl && parentUrl !== 'unknown' && parentUrl !== 'false')
      ? parentUrl
      : (storedParentUrl && storedParentUrl !== 'unknown' && storedParentUrl !== 'false')
        ? storedParentUrl
        : null;
    ...
    // If we ever had a parent URL (stored) or current URL shows iframe params, try navigating back to parent first
    if (fromIframe || hasUrlParams || !!resolvedParentUrl) {
      ...
      try {
        if (resolvedParentUrl) {
          ...
          window.location.href = resolvedParentUrl;
  ...
  // Build a result page URL that includes the selected conversationId and preserves doc context
  const buildResultUrlWithConversation = (conversationId) => {
    try {
      const currentUrl = new URL(window.location.href);
      const params = new URLSearchParams(currentUrl.search);
      // Remove one-time auto-send parameter if lingering
      params.delete('query');
      // Normalize conversation id into URL
      if (conversationId) params.set('conversationId', conversationId);
      // Preserve existing doc context (id, codeName/codename, title) as-is
      // Do not add new params here; only carry forward what's present
      const qs = params.toString();
      return `/resultpage${qs ? `?${qs}` : ''}`;
    } catch {
      // Fallback minimal URL with conversationId only
      return conversationId ? `/resultpage?conversationId=${encodeURIComponent(conversationId)}` : '/resultpage';
    }
  };

  const handleNewChat = () => {
    // Guard: prevent starting a new chat while a manual auto-send is in flight
    if (isNewChatDisabled) {
      console.warn('New Chat is temporarily disabled while your question is being processed.');
      return;
    }
    // DEBUG: Log state before initiating new chat
    try {
      console.group('🆕 New Chat initiation');
      console.log('Pre-state', {
        isNewChatDisabled,
        currentThreadId: currentThread?.id,
        currentThreadTitle: currentThread?.title,
        conversationLength: currentThread?.conversation?.length || 0,
    ...
    // Safety: remove only 'query' from URL; keep conversationId and doc context intact
    try {
      if (window && window.history && typeof window.history.replaceState === 'function') {
        const url = new URL(window.location.href);
        if (url.searchParams.has('query')) {
    ...
    // DEBUG: Post-state after new chat creation
    try {
      console.group('🆕 New Chat created');
      console.log('Post-state', {
        newThreadId: newThread.id,
        manualQueryConsumed: window.__manualQueryConsumed || false,
        sendInProgress: !!window.__sendInProgress,
        urlSearchAfterClear: window.location.search
      });
      console.groupEnd();
    } catch (_) {}
  ...
  const handleThreadSelect = async (thread) => {
    try {
      ...
      setIsNewChatActive(false);
  setUserQuestion('');
  navigate(buildResultUrlWithConversation(thread.id));
  ...
  // Callbacks from ChatPage to control New Chat disabled state
  const handleAutoSendStart = () => {
    setIsNewChatDisabled(true);
  };
  const handleAutoSendComplete = () => {
    // Re-enable New Chat once the auto-submitted manual question finishes its send cycle
    setIsNewChatDisabled(false);
    // Clear any lingering manual question so a subsequent New Chat doesn't re-use it
    setUserQuestion('');
  };
