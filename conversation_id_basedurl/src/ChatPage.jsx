    ...
    const loadConversationFromUrl = async () => {
      ...
      try {
        ...
        if (conversation && conversation.messages) {
          ...
          // Preserve conversationId and doc context; remove only 'query' if present
          try {
            const url = new URL(window.location.href);
            if (url.searchParams.has('query')) {
              url.searchParams.delete('query');
          ...
          // Ensure sidebar highlights this deep-linked conversation by injecting it if missing
          try {
            if (addConversationImmediateRef && addConversationImmediateRef.current && typeof addConversationImmediateRef.current.addConversation === 'function') {
              addConversationImmediateRef.current.addConversation(
                urlConversationId,
                conversation.title || 'Conversation',
                {
                  created_at: conversation.created_at || conversation.createdAt,
                  updated_at: conversation.updated_at || conversation.last_message_at || conversation.updatedAt,
            ...
            console.warn('Sidebar injection for deep-link conversation failed (non-fatal):', e?.message || e);
          }
