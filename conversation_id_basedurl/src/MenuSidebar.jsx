...
const MenuSidebar = ({ onBack, onToggleTheme, isDarkMode, onNewChat, onThreadSelect, currentActiveThread, isNewChatActive, onAddConversationImmediate, onThreadUpdate, isNewChatDisabled }) => {
  ...
  // Optional meta allows stable ordering (preserves original timestamps to prevent jumpiness)
  const addConversationImmediately = (conversationId, title, meta = null) => {
    ...
    const newThread = {
      id: conversationId,
      title: title,
      created_at: meta?.created_at || meta?.createdAt || nowIso,
      updated_at: meta?.updated_at || meta?.last_message_at || meta?.updatedAt || nowIso,
      last_message_at: meta?.last_message_at || meta?.updated_at || meta?.lastMessageAt || nowIso,
    ...
    setAllThreads(prev => {
      ...
      // Remove existing occurrences across groups
      const removeId = arr => arr.filter(t => t && t.id !== conversationId);
      const cleaned = {
        today: removeId(safePrev.today),
        yesterday: removeId(safePrev.yesterday),
        lastWeek: removeId(safePrev.lastWeek),
        last30Days: removeId(safePrev.last30Days)
      };

      // Determine bucket based on created_at (original timestamp) with same boundary logic as loader
      const createdDate = new Date(newThread.created_at);
      const todayStart = new Date(createdDate.getFullYear(), createdDate.getMonth(), createdDate.getDate());
      const now = new Date();
      const globalTodayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
      const yesterdayStart = new Date(globalTodayStart.getTime() - 24 * 60 * 60 * 1000);
      const lastWeekStart = new Date(globalTodayStart.getTime() - 7 * 24 * 60 * 60 * 1000);
      const last30DaysStart = new Date(globalTodayStart.getTime() - 30 * 24 * 60 * 60 * 1000);

      // Use original created_at relative to NOW (not its own day start) to categorize
      let targetGroup = 'last30Days';
      if (createdDate >= globalTodayStart) targetGroup = 'today';
      else if (createdDate >= yesterdayStart) targetGroup = 'yesterday';
      else if (createdDate >= lastWeekStart) targetGroup = 'lastWeek';
      else if (createdDate >= last30DaysStart) targetGroup = 'last30Days';

      cleaned[targetGroup] = [newThread, ...cleaned[targetGroup]];

      // Stable sort each group by updated_at DESC (fallback to created_at)
      ...
      return {
        today: sortGroup(cleaned.today),
        yesterday: sortGroup(cleaned.yesterday),
        lastWeek: sortGroup(cleaned.lastWeek),
        last30Days: sortGroup(cleaned.last30Days)
      };
    });
