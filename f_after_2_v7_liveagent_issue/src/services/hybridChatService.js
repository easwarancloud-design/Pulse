...
export class HybridChatService {
  ...
  /**
   * Save a system message (e.g., control markers like <<LiveAgent>> or UI state notices)
   * Ensures it is persisted via the same conversations/messages API used for user/assistant messages.
   * @param {string} content - The system message content (e.g., '<<LiveAgent>>')
   * @param {object} metadata - Additional metadata to store alongside the message
   * @param {string} [chatId] - Optional chat bubble id to correlate UI and backend
   */
  async saveSystemMessage(content, metadata = {}, chatId = null) {
    try {
      if (!this.currentConversationId) {
        console.warn('⚠️ No active conversation ID when saving system message. Skipping.');
        return;
      }

      // Persist using the same addMessage pipeline with message_type 'system'
      ...
      // Don't throw to avoid breaking UI flows; log and continue
      return null;
    }
  }
