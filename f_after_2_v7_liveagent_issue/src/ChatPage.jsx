...
const ChatPage = ({ onBack, userQuestion, onToggleTheme, isDarkMode, currentThread, isNewChat, isNewChatActive, isNewChatDisabled, onNewChat, onThreadSelect, onFirstMessage, onThreadUpdate, userInfo, addConversationImmediateRef, onAutoSendStart, onAutoSendComplete }) => {
  ...
  // Live agent UI flags
  const [showLiveAgentSwitchBanner, setShowLiveAgentSwitchBanner] = useState(false);
  const [isLiveAgentCardShowing, setIsLiveAgentCardShowing] = useState(false);
  const [liveAgent, setLiveAgent] = useState(false);
  const [rehydratedSelectedGroup, setRehydratedSelectedGroup] = useState(null);
  // Anchor ButtonRow to the assistant message that triggered LiveAgent
  const [liveAgentAnchorChatId, setLiveAgentAnchorChatId] = useState(null);
  const liveAgentRequestIdRef = useRef(null);
    ...
    try {
      ...
      if (responseText.length < 50) {
        ...
        if (/<<\s*LiveAgent\s*>>/i.test(shortText) || /<\s*LiveAgent\s*>/i.test(shortText)) {
          // Do not inject a system message card into the timeline; use standalone ButtonRow instead
          setIsLiveAgentCardShowing(true);
          // Anchor the ButtonRow to this assistant message
          setLiveAgentAnchorChatId(botChatId);
          // Persist the LiveAgent system marker for rehydration
          try {
            const activeConversationIdPersist = hybridChatService.getCurrentConversationId();
            if (activeConversationIdPersist) {
              localConversationManager.saveMessageLocally(activeConversationIdPersist, {
                type: 'system',
                text: '<<LiveAgent>>',
                chat_id: botChatId,
                timestamp: Date.now(),
                metadata: { marker: 'LiveAgent', message_type: 'system' }
              });
              await hybridChatService.saveSystemMessage(
                '<<LiveAgent>>',
                { marker: 'LiveAgent', message_type: 'system' },
                botChatId
              );
            }
          } catch (_) {}
  ...
  const { done, value } = await reader.read();
      ...
      if (liveAgentTriggered) {
        // Do not inject a system message card into the timeline; use standalone ButtonRow instead
        setIsLiveAgentCardShowing(true);
        // Anchor the ButtonRow to the assistant message that triggered the marker
        setLiveAgentAnchorChatId(botChatId);
        // Persist the LiveAgent system marker for rehydration
        try {
          const activeConversationIdPersist2 = hybridChatService.getCurrentConversationId();
          if (activeConversationIdPersist2) {
            localConversationManager.saveMessageLocally(activeConversationIdPersist2, {
              type: 'system',
              text: '<<LiveAgent>>',
              chat_id: botChatId,
              timestamp: Date.now(),
  ...
  // Verify live-agent banner state against server-side session; hide if no active session
  useEffect(() => {
    const verifyUserSession = async () => {
      try {
        if (!RESOLVED_DOMAIN_ID) return;
        // Only verify when banner is showing to avoid extra calls
        if (!showLiveAgentSwitchBanner) return;
        const sessionUrl = API_ENDPOINTS.USER_SESSION(RESOLVED_DOMAIN_ID);
        console.log('🔎 Verifying user session for live-agent banner:', { url: sessionUrl });
        const resp = await fetch(sessionUrl, { method: 'GET', headers: { 'Content-Type': 'application/json' } });
        if (!resp.ok) {
          console.warn('⚠️ USER_SESSION check failed:', resp.status);
          return;
        }
        const data = await resp.json();
        // Expect shape: { active_request_id?: string, conversation_id?: string, active?: boolean }
        const active = !!(data && (data.active || data.active_request_id));
        const activeConv = data && data.conversation_id;
        console.log('🧭 USER_SESSION state:', { active, active_request_id: data?.active_request_id, conversation_id: activeConv });
        if (!active) {
          // No active live-agent session → hide banner and clear refs
          setShowLiveAgentSwitchBanner(false);
          setIsLiveAgentCardShowing(false);
          setLiveAgent(false);
          liveAgentRequestIdRef.current = null;
        } else if (activeConv && currentThread?.id && activeConv === currentThread.id) {
          // Active session belongs to this conversation; show banner only if applicable
          setIsLiveAgentCardShowing(false);
          setLiveAgent(true);
        }
      } catch (err) {
        console.warn('⚠️ USER_SESSION verify error:', err);
      }
    };
    verifyUserSession();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [showLiveAgentSwitchBanner, currentThread?.id, RESOLVED_DOMAIN_ID]);


  // Rehydrate: if a LiveAgent marker was persisted for this conversation, restore the card and selection
  useEffect(() => {
    try {
      const activeId = hybridChatService.getCurrentConversationId() || currentThread?.id;
      if (!activeId) return;
      const localConv = localConversationManager.getLocalConversation(activeId);
      const items = Array.isArray(localConv?.messages) ? localConv.messages : (currentThread?.conversation || []);
      // Find all persisted LiveAgent markers
      const markers = items.filter(m => m?.type === 'system' && typeof m.text === 'string' && /<<\s*LiveAgent\s*>>/i.test(m.text));
      if (!markers.length) return;
      // Prefer most recent actionable (selected/transferred); ignore ended/end_conversation
      const actionable = [...markers].reverse().find(m => {
        const c = (m.text || '').trim();
        const a = (c.split(',')[1] || '').trim();
        return a !== 'end_conversation' && a !== 'ended';
      });
      const ending = [...markers].reverse().find(m => {
        const c = (m.text || '').trim();
        const a = (c.split(',')[1] || '').trim();
        return a === 'end_conversation' || a === 'ended';
      });
      if (!actionable || ending) {
        // No actionable selection or explicit end present → do not show card
        setIsLiveAgentCardShowing(false);
        setLiveAgentAnchorChatId(null);
        return;
      }

      const selected = actionable?.metadata?.selected_option;
      // Show the live agent card again
      setIsLiveAgentCardShowing(true);
      // Anchor next to the assistant message where the marker was saved
      if (actionable?.chat_id) {
        setLiveAgentAnchorChatId(actionable.chat_id);
      }
      // Re-highlight the previously selected option on the card
      if (selected === 'servicenow_catalog' && buttonRowRef.current && typeof buttonRowRef.current.setSelectedGroup === 'function') {
        buttonRowRef.current.setSelectedGroup('HR Service Request');
      }
      // If ServiceNow was chosen previously, append the link bubble again to keep UX consistent
      if (selected === 'servicenow_catalog' && API_ENDPOINTS?.SERVICENOW_HR_CATALOG) {
        const catalogUrl = API_ENDPOINTS.SERVICENOW_HR_CATALOG;
        const linkBubble = (
          <div style={{ textAlign: 'left' }}>
            <a href={catalogUrl} target="_blank" rel="noopener noreferrer" className="sn-link-bubble">
              <img src={ServiceNowIconImg} alt="ServiceNow" style={{ height: 18, width: 18 }} />
              <span>Open ServiceNow ticket catalog in a new tab</span>
            </a>
            <div className="sn-link-bubble-subtext">You can click the button above anytime to view the catalog.</div>
          </div>
        );
        setMessages(prev => [...prev, { id: prev.length + 1, type: 'system', text: linkBubble }]);
        setMessages(prev => [...prev, { id: prev.length + 2, type: 'system', text: 'You can continue chatting here.' }]);
      }
    } catch (_) {}
  }, [currentThread?.id]);
  ...
  // Live Agent session state (already declared above): keep only unique refs here
  // Track the conversation where live agent session started (Option B routing)
  const [liveAgentOriginConversationId, setLiveAgentOriginConversationId] = useState(null);
  ...
  // Rehydrate UI state based on stored system messages with <<LiveAgent>> markers
  useEffect(() => {
    try {
      console.group('♻️ LiveAgent Rehydration');
      console.log('Current thread id:', currentThread?.id);
      const conv = currentThread?.conversation || [];
      if (!Array.isArray(conv) || conv.length === 0) return;

      // Find latest markers
      const markers = conv.filter(m => (m.type === 'system' || m.message_type === 'system') && typeof m.text === 'string' && /<<\s*LiveAgent\s*>>/i.test(m.text));
      console.log('Found LiveAgent markers count:', markers.length, markers);
      if (markers.length === 0) return;

      // Prefer the most recent actionable marker (selected/transferred), not an end marker
      const actionable = [...markers].reverse().find(m => {
        const c = (m.text || '').trim();
        const a = (c.split(',')[1] || '').trim();
        return a !== 'end_conversation' && a !== 'ended';
      ...
      const content = (actionable.text || '').trim();
      console.log('Chosen marker content:', content);
      // Extract parts: <<LiveAgent>>,action,option
      const parts = content.split(',').map(s => s.trim());
      const action = parts[1] || '';
      const option = parts[2] || '';
      console.log('Parsed action/option:', { action, option });

      // Restore request id if present in metadata
  const reqId = actionable?.metadata?.request_id || null;
      if (reqId) {
        liveAgentRequestIdRef.current = reqId;
        console.log('Restored request_id:', reqId);
      }

      // Map options to ButtonRow selection
      const normalizedOption = (option || '').toLowerCase();
      const selectedGroup = (() => {
        if (normalizedOption.includes('manager')) return 'manager_coaching';
        if (normalizedOption.includes('hr')) return 'other_hr_support';
        if (normalizedOption.includes('servicenow')) return 'servicenow_catalog';
        if (normalizedOption.includes('continue')) return 'continue_chatting';
        return null;
      })();
      if (selectedGroup) {
        setRehydratedSelectedGroup(selectedGroup);
        console.log('Rehydrated selectedGroup:', selectedGroup);
      }

      // Drive UI state based on action
  if (action === 'selected') {
        // Show the live agent selection card/buttons again with the selected option highlighted
        setShowLiveAgentSwitchBanner(true);
        console.log('Setting banner visible (selected)');
        setIsLiveAgentCardShowing(true);
        setLiveAgentOriginConversationId(currentThread?.id || null);
        // Keep bot mode active until WS connection is made
        setLiveAgent(false);
        // If ButtonRow supports imperative selection, set it
        try {
          if (buttonRowRef.current && typeof buttonRowRef.current.setSelectedGroup === 'function' && selectedGroup) {
            buttonRowRef.current.setSelectedGroup(selectedGroup);
            console.log('ButtonRow ref setSelectedGroup called');
          }
        } catch (_) {}
      } else if (action === 'transferred') {
        // Show banner indicating transfer; keep live agent mode active if applicable
        setShowLiveAgentSwitchBanner(true);
        console.log('Setting banner visible (transferred)');
        setIsLiveAgentCardShowing(true);
        setLiveAgent(true);
      } else if (action === 'ended' || action === 'end_conversation') {
        // Hide banners and reset live agent state
        setShowLiveAgentSwitchBanner(false);
        console.log('Hiding banner/card (ended)');
        setLiveAgent(false);
        setRehydratedSelectedGroup(null);
        setIsLiveAgentCardShowing(false);
      }
      console.groupEnd();
    } catch (rehydrateErr) {
      console.warn('⚠️ LiveAgent rehydration failed:', rehydrateErr);
    }
  }, [currentThread?.id]);

  // Ensure system messages with LiveAgent markers render plainly (not stripped by formatter)
  const renderSystemMessage = (text) => {
    if (typeof text !== 'string') return text;
    // Show the raw marker line for visibility in history
    return (
      <div style={{
        backgroundColor: 'transparent',
        padding: '8px 12px',
        borderLeft: `4px solid ${isDarkMode ? '#A0BEEA' : '#1a366f'}`,
        borderRadius: '6px',
        color: isDarkMode ? '#A0BEEA' : '#1a366f',
        fontFamily: 'Elevance Sans, Segoe UI, sans-serif',
        fontSize: '14px',
  ...
  // Note: terminateLiveAgent is defined later in the file; removed earlier duplicate.

  const handleLiveAgentConnect = (info) => {
    if (!info) return;

    // ServiceNow catalog: show a friendly link bubble, then a continue message
    if (info.type === 'servicenow_catalog_opened') {
      const catalogUrl = info.url;
      const linkBubble = (
        <div style={{ textAlign: 'left' }}>
          <a
            href={catalogUrl}
            ...
            className="sn-link-bubble"
          >
            <img src={ServiceNowIconImg} alt="ServiceNow" style={{ height: 18, width: 18 }} />
            <span>Open ServiceNow ticket catalog in a new tab</span>
          </a>
          <div className="sn-link-bubble-subtext">
            You can click the button above anytime to view the catalog.
          </div>
        </div>
      );

      // Append the link bubble
      ...
      // Append a follow-up message to continue chat
      setMessages(prev => [...prev, {
        id: prev.length + 2,
        type: 'system',
        text: 'You can continue chatting here.'
      }]);

      // Persist the selection choice tied to the LiveAgent card
      try {
        const activeConversationIdPersist3 = hybridChatService.getCurrentConversationId();
        if (activeConversationIdPersist3) {
          localConversationManager.saveMessageLocally(activeConversationIdPersist3, {
            type: 'system',
            text: '<<LiveAgent>>,selected,ServiceNowCatalog',
            chat_id: `sys_${Date.now()}`,
            timestamp: Date.now(),
            metadata: { marker: 'LiveAgent', selected_option: 'servicenow_catalog', message_type: 'system' }
          });
          // Fire-and-forget without await (this function isn't async)
          hybridChatService
            .saveSystemMessage('<<LiveAgent>>,selected,ServiceNowCatalog', { marker: 'LiveAgent', selected_option: 'servicenow_catalog', message_type: 'system' })
            .catch(() => {});
        }
      } catch (_) {}

      // Keep the LiveAgent card visible after opening catalog (buttons remain for further actions)
      setIsLiveAgentCardShowing(true);
      return;
    }
    ...
    if (info.type === 'continue') {
      ...
      // Persist the selection as a system message
      try {
        const convId = hybridChatService.getCurrentConversationId();
        if (convId) {
          const chatId = `sys_${Date.now()}`;
          localConversationManager.saveMessageLocally(convId, {
            type: 'system',
            text: '<<LiveAgent>>,selected,ContinueChatting',
            chat_id: chatId,
            timestamp: Date.now(),
            metadata: { marker: 'LiveAgent', selected_option: 'continue_chatting', message_type: 'system' }
          });
          hybridChatService.saveSystemMessage('<<LiveAgent>>,selected,ContinueChatting', { marker: 'LiveAgent', selected_option: 'continue_chatting', message_type: 'system' }, chatId).catch(() => {});
        }
      } catch (_) {}
      // Keep the LiveAgent card visible after continue selection
      setIsLiveAgentCardShowing(true);
      return;
    }
    ...
    if (info.type === 'connecting') {
      ...
      // Persist which live agent option was selected
      try {
        const convId = hybridChatService.getCurrentConversationId();
        if (convId && info.group) {
          const normalized = info.group === 'AgenticContactCenter' ? 'other_hr_support' : (info.group === 'AgenticHRAdvisor' ? 'manager_coaching' : info.group);
          const chatId = `sys_${Date.now()}`;
          localConversationManager.saveMessageLocally(convId, {
            type: 'system',
            text: `<<LiveAgent>>,selected,${normalized}`,
            chat_id: chatId,
            timestamp: Date.now(),
            metadata: { marker: 'LiveAgent', selected_option: normalized, message_type: 'system' }
          });
          hybridChatService.saveSystemMessage(`<<LiveAgent>>,selected,${normalized}`, { marker: 'LiveAgent', selected_option: normalized, message_type: 'system' }, chatId).catch(() => {});
        }
      } catch (_) {}
      // Keep the LiveAgent card visible while connecting
      setIsLiveAgentCardShowing(true);
      return;
    }
    ...
    if (info.type === 'transferred' && info.requestId) {
      ...
      try {
        const originId = hybridChatService.getCurrentConversationId() || currentThread?.id || null;
        setLiveAgentOriginConversationId(originId);
        // Persist transfer event with group for traceability
        if (originId && info.group) {
          const normalized = info.group === 'AgenticContactCenter' ? 'other_hr_support' : (info.group === 'AgenticHRAdvisor' ? 'manager_coaching' : info.group);
          const chatId = `sys_${Date.now()}`;
          localConversationManager.saveMessageLocally(originId, {
            type: 'system',
            text: `<<LiveAgent>>,transferred,${normalized}`,
            chat_id: chatId,
            timestamp: Date.now(),
            metadata: { marker: 'LiveAgent', selected_option: normalized, request_id: info.requestId, message_type: 'system' }
          });
          hybridChatService.saveSystemMessage(`<<LiveAgent>>,transferred,${normalized}`, { marker: 'LiveAgent', selected_option: normalized, request_id: info.requestId, message_type: 'system' }, chatId).catch(() => {});
        }
      } catch (_) {}
      return;
    }
    ...
    if (info.type === 'error') {
      setMessages(prev => [...prev, { id: prev.length + 1, type: 'system', text: info.message }]);
      // Keep the LiveAgent card visible on error to allow retry
      setIsLiveAgentCardShowing(true);
    }
  };
  ...
  const connectWebSocket = (requestId, domainid) => {
    try {
      ...
      liveAgentRequestIdRef.current = requestId;
      setLiveAgent(true);
      setChatEnded(false);
      ...
      ws.onmessage = (event) => {
        try {
          // Debug: log raw websocket data for live agent
          console.log('[LiveAgent][WS] raw event:', event);
          console.log('[LiveAgent][WS] raw data:', event?.data);
          ...
          setMessages(prev => ([...prev, {
            ...
            text: renderLiveAgentMessage(text, isFirstMessage(), agentName)
              ...
              // Persist live agent inbound message to backend with message_type 'live_agent'
              try {
                const convId = hybridChatService.getCurrentConversationId() || currentThread?.id;
                if (convId && text) {
                  const chatId = `agent_${Date.now()}`;
                  // Fire-and-forget persist (ws handler is sync)
                  conversationStorage.addMessage(
                    convId,
                    'live_agent',
                    text,
                    { source: 'live_agent_ws', agent_name: agentName || null, timestamp: new Date().toISOString() },
                    [],
                    chatId
                  ).catch(() => {});
                  localConversationManager.saveMessageLocally(convId, {
                    type: 'live_agent', text, chat_id: chatId, timestamp: Date.now()
                  });
                }
              } catch (persistErr) {
                console.warn('⚠️ Failed to persist live agent inbound message:', persistErr?.message || persistErr);
              }
        } catch (err) {
  ...
  // Helper: End live agent via API then cleanup
  const endLiveAgentConversation = async ({ domainId, emailId, userId }) => {
    try {
      const requestId = liveAgentRequestIdRef.current || requestIdRef.current;
      if (!requestId) return;
      const payload = {
        requestId,
        token: 'vaacubed',
        ...
        clientSessionId: '',
        silentMessage: false,
        message: { text: 'You have ended the conversation.', typed: true },
        userId,
        emailId,
        timestamp: Date.now(),
      ...
      console.error('Failed to send END_CONVERSATION:', err);
    } finally {
      ...
      liveAgentRequestIdRef.current = null;
      setLiveAgent(false);
      setLiveAgentOriginConversationId(null);
      setShowLiveAgentSwitchBanner(false);
    }
  };

  // UI: Soft confirm banner (Option B) shown when user is in a different conversation
  const LiveAgentSwitchBanner = ({ onEnd, onKeep }) => (
    <div className="bg-yellow-50 border border-yellow-300 text-yellow-900 px-3 py-2 rounded mb-2 flex items-center gap-3">
      <span>You have an active live agent session in another conversation. Messages here will go to AI. End the agent session?</span>
      <button className="px-2 py-1 bg-red-600 text-white rounded" onClick={onEnd}>End session</button>
      <button className="px-2 py-1 bg-gray-200 rounded" onClick={onKeep}>Keep session</button>
    </div>
  );

  // Guard: Before sending a message, decide routing based on conversation and live agent state
  const sendMessage = async ({ text, conversationId, domainId, emailId, userId }) => {
    const inLiveAgentThread = liveAgent && liveAgentOriginConversationId && conversationId === liveAgentOriginConversationId;
    if (inLiveAgentThread) {
      // Route to live agent via USER_TO_AGENT
      const requestId = liveAgentRequestIdRef.current || requestIdRef.current;
      if (!requestId) {
        console.warn('Live agent active but missing requestId; falling back to AI');
        return sendMessageToAI({ text, conversationId, domainId });
      }
      const payload = {
        requestId,
        token: 'vaacubed',
      ...
      try {
        ...
        // Persist the user's message in the conversation with message_type 'user'
        try {
          const chatId = `user_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
          await hybridChatService.saveUserQuestion(
            text,
            { source: 'live_agent_route', route: 'USER_TO_AGENT', request_id: requestId, timestamp: Date.now() },
            chatId
          );
          const activeId = hybridChatService.getCurrentConversationId() || conversationId;
          if (activeId) {
            localConversationManager.saveMessageLocally(activeId, {
              type: 'user', text, chat_id: chatId, timestamp: Date.now()
            });
          }
        } catch (_) {}
      } catch (err) {
        console.error('Failed to send message to live agent:', err);
        // Optionally show error toast and keep message locally
      }
      return;
    }

    // Different conversation while agent is active → show banner and route to AI
    if (liveAgent && liveAgentOriginConversationId && conversationId !== liveAgentOriginConversationId) {
      setShowLiveAgentSwitchBanner(true);
      return sendMessageToAI({ text, conversationId, domainId });
    }
  ...
  // Placeholder: existing AI message sender
  const sendMessageToAI = async ({ text, conversationId, domainId }) => {
    // ...existing code...
  };

  // Persist termination marker to backend/local, then cleanup
  const postSystemAndTerminate = (message, domainid) => {
    ...
    // Persist termination marker to backend/local for history
    try {
      const convId = hybridChatService.getCurrentConversationId() || currentThread?.id;
      const reqId = liveAgentRequestIdRef.current || requestIdRef.current || null;
      if (convId) {
        const chatId = `sys_${Date.now()}`;
        const content = '<<LiveAgent>>,ended';
        const metadata = { marker: 'LiveAgent', reason: message, request_id: reqId, message_type: 'system' };
        // Backend persist (fire-and-forget)
        hybridChatService.saveSystemMessage(content, metadata, chatId).catch(() => {});
        // Local mirror
        localConversationManager.saveMessageLocally(convId, {
          type: 'system', text: content, chat_id: chatId, timestamp: Date.now(), metadata
        });
      }
    } catch (_) {}

    // Cleanup WS and session
    terminateLiveAgent(message, domainid);
  };
  
  const terminateLiveAgent = async (reason = "Disconnected from live agent.", domainidParam) => {
    try {
      // Persist explicit end marker (distinct from postSystemAndTerminate) for clarity
      const convId = hybridChatService.getCurrentConversationId() || currentThread?.id;
      const reqId = liveAgentRequestIdRef.current || requestIdRef.current || null;
      if (convId) {
        const chatId = `sys_${Date.now()}`;
        const content = '<<LiveAgent>>,end_conversation';
        const metadata = { marker: 'LiveAgent', reason, request_id: reqId, message_type: 'system' };
        hybridChatService.saveSystemMessage(content, metadata, chatId).catch(() => {});
        localConversationManager.saveMessageLocally(convId, {
          type: 'system', text: content, chat_id: chatId, timestamp: Date.now(), metadata
        });
    ...
    // Existing logic: send END to backend, close WS, reset state
    try {
      const requestId = liveAgentRequestIdRef.current || requestIdRef.current;
      if (!requestId) {
  ...
  const handleSendMessage = async () => {
    if (userInput.trim() && !loading && !showConfirm) {
      ...
      // Determine current conversation id for routing
      const currentConversationId = (() => {
        try { return hybridChatService.getCurrentConversationId() || currentThread?.id || null; } catch { return currentThread?.id || null; }
      })();

      // If live agent is active in this conversation, route to USER_TO_AGENT without AI streaming
      if (liveAgent && liveAgentOriginConversationId && currentConversationId === liveAgentOriginConversationId) {
        // Append only the user message; do not create an assistant placeholder
        const userChatId = `user_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        ...
        try {
          const info = JSON.parse(localStorage.getItem('userInfo') || '{}');
          email = info.email || info.emailId || email;
        } catch {}
        try {
          await sendMessage({
            text: questionToSend,
            ...
            userId: RESOLVED_DOMAIN_ID,
            emailId: email,
          });
        } catch (err) {
          console.error('Failed to route user message to live agent:', err);
        }
        return;
      }

      // If live agent is active in another conversation, show banner and continue with AI here
      if (liveAgent && liveAgentOriginConversationId && currentConversationId !== liveAgentOriginConversationId) {
        setShowLiveAgentSwitchBanner(true);
      }

      // Use workforce agent (AI) for normal responses
  ...
  <div className={`flex-1 flex flex-col h-full overflow-hidden relative z-10 ${isDarkMode ? 'bg-gradient-to-br from-[#072056] to-[#000B23]' : ''}`}>
        ...
        <div className={`flex-1 overflow-y-auto p-5`}>
          ...
          >
            ...
            {hasMoreMessages && (
              ...
              {/* Rehydrated LiveAgent selection card (fallback at top only if no anchor) */}
              {isLiveAgentCardShowing && !liveAgentAnchorChatId && (
                <>
                {(() => { try { console.log('Rendering ButtonRow (rehydrated)', { initialSelectedGroup: rehydratedSelectedGroup, liveAgent, banner: showLiveAgentSwitchBanner }); } catch {} return null; })()}
                <div style={{
                  ...
                  <ButtonRow
                    ...
                    initialSelectedGroup={rehydratedSelectedGroup}
                  />
                </div>
                  </>
              )}
                ...
                {message.type === 'user' ? (
                  ...
                  <div>
                    <div className="space-y-4">
                      ...
                      {/* Inline LiveAgent buttons: render directly after the assistant message that triggered it */}
                      {isLiveAgentCardShowing && liveAgentAnchorChatId && message.type === 'assistant' && message.chat_id === liveAgentAnchorChatId && (
                        <div style={{
                          backgroundColor: isDarkMode ? "#122F65" : "#f9fbfc",
                          padding: "20px",
                          borderRadius: "12px",
        ...
        <div className={`p-5 ${isDarkMode ? '' : 'bg-[#F9FAFB] border-gray-200 border-t'} flex-shrink-0`}>
          {/* Soft confirm banner when typing in a different conversation while live agent is active */}
          {showLiveAgentSwitchBanner && (
            <div className="max-w-[760px] mx-auto mb-3">
              <LiveAgentSwitchBanner
                onEnd={() => {
                  // Resolve identifiers from localStorage/userInfo
                  let email = 'user@email.com';
                  try {
                    const info = JSON.parse(localStorage.getItem('userInfo') || '{}');
                    email = info.email || info.emailId || email;
                  } catch {}
                  endLiveAgentConversation({
                    domainId: RESOLVED_DOMAIN_ID,
                    emailId: email,
                    userId: RESOLVED_DOMAIN_ID,
                  });
                }}
                onKeep={() => setShowLiveAgentSwitchBanner(false)}
              />
            </div>
          )}
