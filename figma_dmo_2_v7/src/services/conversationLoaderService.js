...
export class ConversationLoaderService {
  ...
  async fetchConversationFromAPI(conversationId, messageOffset = 0, messageLimit = 50, includeMessages = true, retries = 3) {
    // 🚫 Guard: skip API attempts for temporary placeholder IDs (thread_<timestamp>) that are not persisted yet.
    // These are created client-side for optimistic UI; backend only knows 'conv_' IDs.
    if (conversationId && /^thread_\d+/.test(conversationId)) {
      console.log('🛑 fetchConversationFromAPI: Skipping API fetch for placeholder thread id:', conversationId);
      return null;
    }
